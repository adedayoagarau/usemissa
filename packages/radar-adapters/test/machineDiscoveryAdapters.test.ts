...
test("Sundance deadlines emits current official application cards", async () => {
  ...
  const upcomingLabel = upcoming.toLocaleDateString("en-US", {
    year: "numeric",
    month: "long",
    day: "numeric",
    timeZone: "UTC",
  ...
  const html = `
    ...
    <p>Extended Deadline: ${upcomingLabel}</p>
  ...
  try {
    ...
    assert.equal(result.links[0]?.discoveryMachineRecord?.deadlineDate, upcomingIso);
