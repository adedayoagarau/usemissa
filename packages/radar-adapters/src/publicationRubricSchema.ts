...
begin
  ...
  select coalesce(s.url <> '', true) into source_ok
  ...
  freshness_ok := (
    new.status = 'opening-soon' and (new.open_date is null or new.open_date >= current_date)
  ) or (
...
export async function ensurePublicationRubricSchema(pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      check.rows[0].prosrc.includes("coalesce(s.url <> '', true)")
    ) {
