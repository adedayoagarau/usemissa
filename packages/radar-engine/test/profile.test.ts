...
test("profile updates trim public fields and derive completeness", () => {
  ...
  assert.deepEqual(engine.profileCompleteness(user.id), {
    complete: true,
    missing: [],
  });
});
...
test("profile consequence motion events are durable and once-only", () => {
  const { engine, user } = engineWithUser();
  const first = engine.markProfileMotion(
    user.id,
    "first-sample-published",
    "2026-08-14T12:00:00.000Z",
  );
  const repeat = engine.markProfileMotion(
    user.id,
    "first-sample-published",
  ...
  assert.deepEqual(user.profileMotion, {
    "first-sample-published": "2026-08-14T12:00:00.000Z",
  });
});
...
test("profile completeness calls out unconfigured opportunity preferences", () => {
  ...
  assert.deepEqual(engine.profileCompleteness(user.id), {
    complete: false,
    missing: ["opportunityPreferences"],
  });
});
...
test("opportunity preferences are normalized and validated", () => {
  ...
  assert.throws(
    () =>
      engine.updateUserProfile(user.id, {
        opportunityPreferences: { types: ["workshop"] },
      }),
    (error: unknown) =>
      error instanceof ProfileValidationError &&
      error.field === "opportunityPreferences",
  );
});
...
test("profile taxonomy preferences are private, canonical-id based, and validated", () => {
  ...
  engine.updateUserProfile(user.id, {
    taxonomyPreferences: [
      { termId: "term_poetry", preference: "include", weight: 100 },
    ],
  });
  assert.deepEqual(user.taxonomyPreferences, [
    { termId: "term_poetry", preference: "include", weight: 100 },
  ]);
  assert.equal(
    Object.prototype.hasOwnProperty.call(
      engine.publicUserProfile(user.id),
      "taxonomyPreferences",
    ),
    false,
  );
  assert.throws(
    () =>
      engine.updateUserProfile(user.id, {
        taxonomyPreferences: [
          { termId: "term_poetry", preference: "include", weight: 101 },
        ],
      }),
    (error: unknown) =>
      error instanceof ProfileValidationError &&
      error.field === "taxonomyPreferences",
  );
});
...
test("profile update rejects blank or over-limit values without partial mutation", () => {
  const { engine, user } = engineWithUser();
  assert.throws(
    () => engine.updateUserProfile(user.id, { displayName: "   " }),
    (error: unknown) =>
      error instanceof ProfileValidationError && error.field === "displayName",
  );
  assert.equal(user.displayName, "  Ada  ");
  assert.throws(
    () => engine.updateUserProfile(user.id, { bio: "x".repeat(1001) }),
    (error: unknown) =>
      error instanceof ProfileValidationError && error.field === "bio",
  );
  assert.equal(user.bio, "  A writer.  ");
  assert.throws(
    () => engine.updateUserProfile(user.id, { displayName: "x".repeat(121) }),
    (error: unknown) =>
      error instanceof ProfileValidationError && error.field === "displayName",
  );
});
...
test("public profile projection excludes matching and account data and supports missing users", () => {
  ...
  assert.equal(
    Object.prototype.hasOwnProperty.call(profile, "attributes"),
    false,
  );
...
test("privacy defaults fail closed and public projection only includes opted-in fields", () => {
  const { engine, user } = engineWithUser();
  assert.deepEqual(engine.profilePrivacy(user.id), {
    displayName: "public",
    bio: "public",
    trackedOpportunityCount: "private",
  });
  user.privacy = {
    displayName: "unexpected" as never,
    bio: "unexpected" as never,
    trackedOpportunityCount: "public",
  };
...
test("privacy updates are strict, complete, and no-op safe", () => {
  const { engine, user } = engineWithUser();
  const first = engine.updateProfilePrivacy(user.id, {
    bio: "private",
    trackedOpportunityCount: "public",
  });
  ...
  assert.throws(
    () => engine.updateProfilePrivacy(user.id, { bio: "hidden" as never }),
    ProfilePrivacyValidationError,
  );
  assert.throws(
    () => engine.updateProfilePrivacy(user.id, { future: "public" } as never),
    ProfilePrivacyValidationError,
  );
