...
import {
  cleanupEmailCandidates as cleanupEmailReviewCandidates,
...
import {
  addOpportunityToCustomList as addToList,
  customListsForOpportunity,
  customListsForUser,
  createCustomList,
  deleteCustomList,
  opportunitiesForCustomList,
  removeOpportunityFromCustomList,
  updateCustomList,
} from "./lists/lists.js";
...
async function fetchSources(
    ...
    while (next < sources.length) {
      ...
      try {
        ...
        results[index] = {
          status: "error",
          content: "",
          failureReason: error instanceof Error ? "network" : "unknown",
        };
      }
    }
  };
  await Promise.all(
    Array.from(
      { length: Math.min(fetchConcurrency(), sources.length) },
      worker,
    ),
  );
  return results;
...
export class ProfileValidationError extends Error {
  readonly field:
    "displayName" | "bio" | "taxonomyPreferences" | "opportunityPreferences";
...
function normalizedPrivacy(value: unknown): ProfilePrivacySettings {
  ...
  return {
    displayName:
      stored.displayName === undefined
        ? DEFAULT_PROFILE_PRIVACY.displayName
        : visibility(stored.displayName),
    bio:
      stored.bio === undefined
        ? DEFAULT_PROFILE_PRIVACY.bio
        : visibility(stored.bio),
    trackedOpportunityCount:
      stored.trackedOpportunityCount === undefined
        ? DEFAULT_PROFILE_PRIVACY.trackedOpportunityCount
        : visibility(stored.trackedOpportunityCount),
  };
}

function validatedPrivacyPatch(
  patch: ProfilePrivacyPatch,
): ProfilePrivacyPatch {
  if (!patch || typeof patch !== "object" || Array.isArray(patch))
    throw new ProfilePrivacyValidationError(
      "Privacy settings must be an object.",
    );
  const entries = Object.entries(patch as Record<string, unknown>);
  if (
    entries.some(([key]) => !(PRIVACY_KEYS as readonly string[]).includes(key))
  )
    throw new ProfilePrivacyValidationError(
      "Only supported profile visibility settings can be changed.",
    );
  if (entries.some(([, value]) => value !== "public" && value !== "private"))
    throw new ProfilePrivacyValidationError(
...
function normalizedStringList(
  ...
  if (
    ...
    throw new ProfileValidationError(
      "opportunityPreferences",
      `${field} must be a list of at most ${max} text values.`,
    );
  }
  const values = value.map((item) => item.trim()).filter(Boolean);
  if (values.some((item) => item.length > 120))
    throw new ProfileValidationError(
      "opportunityPreferences",
      `${field} values must be 120 characters or fewer.`,
    );
  return [...new Set(values)];
}

function normalizedOpportunityPreferences(
  user: UserProfile,
  patch: OpportunityPreferencesPatch,
): OpportunityPreferences {
  const current = user.opportunityPreferences ?? EMPTY_OPPORTUNITY_PREFERENCES;
  const next = { ...current, ...patch } as Record<string, unknown>;
  const types = normalizedStringList(
    "types",
    next.types,
    16,
  ) as OpportunityType[];
  if (types.some((type) => !OPPORTUNITY_PREFERENCE_TYPES.has(type)))
    throw new ProfileValidationError(
      "opportunityPreferences",
      "Choose supported opportunity types.",
    );
  const careerStages = normalizedStringList(
    "careerStages",
    next.careerStages,
    3,
  );
  if (careerStages.some((stage) => !CAREER_STAGES.has(stage)))
    throw new ProfileValidationError(
      "opportunityPreferences",
      "Choose supported career stages.",
    );
  const maxFeeCents = next.maxFeeCents;
  if (
    maxFeeCents !== undefined &&
    maxFeeCents !== null &&
    (typeof maxFeeCents !== "number" ||
      !Number.isInteger(maxFeeCents) ||
      maxFeeCents < 0)
  )
    throw new ProfileValidationError(
      "opportunityPreferences",
      "Maximum fee must be a non-negative whole number of cents.",
    );
  const deadlineWithinDays = next.deadlineWithinDays;
  if (
    deadlineWithinDays !== undefined &&
    deadlineWithinDays !== null &&
    (typeof deadlineWithinDays !== "number" ||
      !Number.isInteger(deadlineWithinDays) ||
      deadlineWithinDays < 0 ||
      deadlineWithinDays > 366)
  )
    throw new ProfileValidationError(
      "opportunityPreferences",
      "Deadline window must be between 0 and 366 days.",
    );
  if (
    typeof next.noFeeOnly !== "boolean" ||
    typeof next.simultaneousRequired !== "boolean"
  )
    throw new ProfileValidationError(
      "opportunityPreferences",
      "Fee and simultaneous-submission preferences must be boolean values.",
    );
  return {
    types,
...
function normalizedProfileValues(
  ...
  if (patch.taxonomyPreferences !== undefined) {
    if (
      !Array.isArray(patch.taxonomyPreferences) ||
      patch.taxonomyPreferences.length > 64
    )
      throw new ProfileValidationError(
        "taxonomyPreferences",
        "Choose no more than 64 practice preferences.",
      );
    const seen = new Set<string>();
    taxonomyPreferences = patch.taxonomyPreferences.map((preference) => {
      if (
        !preference ||
        typeof preference.termId !== "string" ||
        !preference.termId.trim()
      )
        throw new ProfileValidationError(
          "taxonomyPreferences",
          "Each preference needs a canonical term.",
        );
      if (!["include", "prefer", "exclude"].includes(preference.preference))
        throw new ProfileValidationError(
          "taxonomyPreferences",
          "Preference must be include, prefer, or exclude.",
        );
      if (
        ...
        throw new ProfileValidationError(
          "taxonomyPreferences",
          "Preference weight must be between 0 and 100.",
        );
      const termId = preference.termId.trim();
      if (seen.has(termId))
        throw new ProfileValidationError(
          "taxonomyPreferences",
          "A canonical term may only appear once.",
        );
      seen.add(termId);
      return {
        termId,
  ...
  const opportunityPreferences =
    patch.opportunityPreferences === undefined
...
export class RadarEngine {
  ...
  addOrganization(
    org: Omit<Organization, "id"> & { id?: string },
  ): Organization {
  ...
  updateUserProfile(userId: string, patch: UserProfilePatch): UserProfile {
    ...
    user.bio = values.bio;
    if (values.taxonomyPreferences !== undefined)
      user.taxonomyPreferences = values.taxonomyPreferences;
    if (values.opportunityPreferences !== undefined)
      user.opportunityPreferences = values.opportunityPreferences;
    return user;
  }
  ...
  publicUserProfile(userId: string): PublicUserProfile | undefined {
    ...
    if (settings.displayName === "public" && displayName)
  ...
  updateProfilePrivacy(
    ...
    changedFields: Array<keyof ProfilePrivacySettings>;
  } {
    const user = this.store.users.get(userId);
    if (!user) throw new Error(`Unknown user: ${userId}`);
    ...
    const changedFields = PRIVACY_KEYS.filter(
  ...
  profileCompleteness(userId: string): {
    complete: boolean;
    missing: Array<"displayName" | "bio" | "opportunityPreferences">;
  } {
    const user = this.store.users.get(userId);
    if (!user)
      return {
        complete: false,
        missing: ["displayName", "bio", "opportunityPreferences"],
      };
  ...
  followOrganization(userId: string, organizationId: string): void {
    if (
      !this.store.follows.some(
        (f) => f.userId === userId && f.organizationId === organizationId,
      )
    ) {
      this.store.follows.push({
        userId,
        organizationId,
        followedAt: this.clock.now().toISOString(),
      });
    }
  }
  ...
  linkTrackedOpportunityToWork(
    ...
    return linkTrackedOpportunityToWork(
      this.ctx,
      userId,
      opportunityId,
      workId,
  ...
  exportTracker(userId: string, now = this.clock.now()): TrackerExportV1 {
    ...
    const tracker = this.store.tracked
      ...
      .sort(
        (a, b) =>
          a.trackedAt.localeCompare(b.trackedAt) ||
      ...
      .map((tracked) => {
        ...
        return {
          ...
          ...(opportunity.fields.deadline.date
      ...
      .concat(
        this.store.manualTrackerEntries
  ...
  organizationSeatUsage(
    organizationId: string,
  ): ReturnType<typeof organizationSeatUsage> {
  ...
  forwardingAddress(userId: string): ForwardingAddressView {
    return forwardingAddressView(this.store, userId);
  }
  createForwardingAddress(userId: string): {
    address: string;
    created: boolean;
    view: ForwardingAddressView;
  } {
    const result = createOrGetForwardingAddress(
      this.store,
      userId,
      this.clock.now(),
      this.ids,
    ...
    return {
      ...
      view: forwardingAddressView(this.store, userId),
    };
  }
  rotateForwardingAddress(
    userId: string,
    idempotencyKey?: string,
  ): { address: string; view: ForwardingAddressView } {
    const result = rotateForwardingAddress(
      this.store,
      userId,
      this.clock.now(),
      this.ids,
  ...
  setForwardingAddressStatus(
    ...
    setForwardingAddressStatus(this.store, userId, status);
    return forwardingAddressView(this.store, userId);
  }
  revokeForwardingAddress(
    userId: string,
    deletePending = false,
  ): { deletedCandidates: number; view: ForwardingAddressView } {
    const result = revokeForwardingAddress(
      this.store,
      userId,
      this.clock.now(),
      deletePending,
    );
    return { ...result, view: forwardingAddressView(this.store, userId) };
  }
  ingestInboundEmail(
  ...
  emailCandidates(
    ...
    return listEmailCandidates(this.store, userId, state, classification);
  }
  reviewEmailCandidate(
    userId: string,
    candidateId: string,
  ...
  cleanupEmailCandidates(): number {
    return cleanupEmailReviewCandidates(this.store, this.clock.now());
  }
  createGmailOAuthState(userId: string, config?: Partial<GmailOAuthConfig>) {
    return createGmailOAuthState(
      this.store,
      userId,
      config,
      this.clock.now(),
  ...
  connectGmail(userId: string, exchange: GmailTokenExchange): GmailConnection {
    return createGmailConnection(
      this.store,
      userId,
      exchange,
      this.clock.now(),
  ...
  gmailConnection(userId: string): GmailConnection | undefined {
    return this.store.gmailConnections.find(
      (item) => item.userId === userId && item.status !== "disconnected",
    );
  }
  queueGmailSync(
    userId: string,
    ...
    const connection = this.gmailConnection(userId);
    if (!connection) throw new Error("Gmail is not connected.");
    return queueGmailSyncJob(
      this.store,
      connection,
      trigger,
      dedupeKey,
  ...
  leaseGmailSyncJob(jobId: string) {
    return leaseGmailSyncJob(this.store, jobId, this.clock.now());
  }
  completeGmailSyncJob(
    jobId: string,
    result: GmailSyncJob["result"],
    targetHistoryId?: string,
  ) {
    return completeGmailSyncJob(
      this.store,
      jobId,
      result,
      targetHistoryId,
  ...
  failGmailSyncJob(jobId: string, errorCode: string) {
    return failGmailSyncJob(this.store, jobId, errorCode, this.clock.now());
  }
  cleanupGmailOAuthStates() {
    return cleanupGmailOAuthStates(this.store, this.clock.now());
  }
  setGmailMode(
    userId: string,
    mode: GmailMode,
    ...
    return setGmailMode(this.store, userId, mode, confirmation, idempotencyKey);
  }
  disconnectGmail(userId: string, deletePending = false) {
    return disconnectGmail(this.store, userId, deletePending, this.clock.now());
  }
  ingestGmailEnvelope(connectionId: string, envelope: InboundEmailEnvelope) {
    const connection = this.store.gmailConnections.find(
      (item) => item.id === connectionId && item.status === "active",
    );
    if (!connection)
      return { accepted: false as const, reason: "unavailable" as const };
    return ingestGmailEnvelope(
      this.store,
      connection,
      envelope,
      this.clock.now(),
  ...
  gmailAutopilotGate(candidateId: string) {
    const candidate = this.store.emailCandidates.find(
      (item) => item.id === candidateId,
    );
    if (!candidate?.gmailConnectionId)
      return { allowed: false, reason: "This is not a Gmail candidate." };
    const connection = this.store.gmailConnections.find(
      (item) => item.id === candidate.gmailConnectionId,
    );
    if (!connection)
      return { allowed: false, reason: "Gmail is no longer connected." };
    return gmailAutopilotGate(this.store, connection, candidate);
  }
  applyGmailAutopilotCandidate(candidateId: string) {
    const candidate = this.store.emailCandidates.find(
      (item) => item.id === candidateId,
    );
    if (
      !candidate?.gmailConnectionId ||
      !candidate.proposedStatus ||
      candidate.candidates.length !== 1
    )
      throw new Error("Gmail candidate is not eligible for Autopilot.");
    const gate = this.gmailAutopilotGate(candidateId);
    if (!gate.allowed) throw new Error(gate.reason);
    return this.reviewEmailCandidate(candidate.userId, candidateId, {
      kind: "confirm",
      opportunityId: candidate.candidates[0]!.opportunityId,
      status: candidate.proposedStatus,
      idempotencyKey: `autopilot:${candidateId}`,
    });
  }
  library(userId: string) {
    return libraryForUser(this.store, userId);
  }
  createLibraryWork(
    userId: string,
    input: {
      ...
      description?: unknown;
      fileId?: unknown;
      taxonomyTermIds?: unknown;
    },
  ): LibraryWork {
    return createLibraryWork(
      this.store,
      userId,
      input,
      this.clock.now(),
  ...
  deleteLibraryWork(userId: string, workId: string): void {
    deleteLibraryWork(this.store, userId, workId);
  }
  createLibraryFile(
    userId: string,
    input: {
  ...
  deleteLibraryFile(userId: string, fileId: string): void {
    deleteLibraryFile(this.store, userId, fileId);
  }
  createSavedAnswer(
    userId: string,
    input: { name: unknown; body: unknown },
  ): SavedAnswer {
    return createSavedAnswer(
      this.store,
      userId,
      input,
      this.clock.now(),
  ...
  deleteSavedAnswer(userId: string, answerId: string): void {
    deleteSavedAnswer(this.store, userId, answerId);
  }

  opportunityChecklist(
    userId: string,
    opportunityId: string,
  ): OpportunityChecklistView {
    return getOpportunityChecklist(
      this.store,
      userId,
      opportunityId,
      this.clock.now(),
  ...
  refreshOpportunityChecklist(
    ...
    return refreshOpportunityChecklist(
      this.store,
      userId,
      opportunityId,
      this.clock.now(),
  ...
  checklistForUser(userId: string): OpportunityChecklistView[] {
    return checklistForUser(this.store, userId);
  }
  ...
  deleteChecklistItem(userId: string, itemId: string): void {
    deleteChecklistItem(this.store, userId, itemId, this.clock.now());
  }

  lists(userId: string, includeArchived = false): CustomList[] {
    return customListsForUser(this.store, userId, includeArchived);
  }
  createList(
    userId: string,
    input: { name: unknown; description?: unknown; colorToken?: unknown },
  ): CustomList {
  ...
  deleteList(userId: string, listId: string): void {
    deleteCustomList(this.store, userId, listId);
  }
  listMemberships(userId: string, listId?: string): CustomListMembership[] {
    return this.store.customListMemberships
      ? [...this.store.customListMemberships.values()].filter(
          (m) =>
            m.userId === userId &&
            (listId === undefined || m.listId === listId),
        )
      : [];
  }
  listsForOpportunity(userId: string, opportunityId: string): CustomList[] {
    return customListsForOpportunity(this.store, userId, opportunityId);
  }
  addToList(
    userId: string,
    listId: string,
  ...
  removeFromList(userId: string, listId: string, opportunityId: string): void {
    removeOpportunityFromCustomList(this.store, userId, listId, opportunityId);
  }
  opportunitiesInList(userId: string, listId: string): Opportunity[] {
    return opportunitiesForCustomList(this.store, userId, listId);
  }
  ...
  async tick(opts?: {
    ...
    const due = dueSources(this.store.sources.values(), now).filter(
      (source) => {
        if (
          opts?.minRegistryTier !== undefined &&
          (source.registryTier ?? 0) < opts.minRegistryTier
        )
          return false;
        if (
          opts?.maxRegistryTier !== undefined &&
          (source.registryTier ?? 0) > opts.maxRegistryTier
        )
          return false;
        return true;
      },
    ...
    const sourcesToProcess =
      opts?.maxSources !== undefined ? due.slice(0, opts.maxSources) : due;
    ...
    for (const [index, source] of sourcesToProcess.entries()) {
      ...
      if (result.status === "error") {
        ...
        report.failedFetchesByReason[reason] =
          (report.failedFetchesByReason[reason] ?? 0) + 1;
      ...
      try {
        ...
        if (hasFatalIssues(candidate) || !looksLikeOpportunity(candidate)) {
          source.consecutiveProcessingFailures =
            (source.consecutiveProcessingFailures ?? 0) + 1;
          report.sourcesFailed++;
          report.processingFailures++;
          report.extractionFailures++;
          report.extractionFailuresByReason.validation =
            (report.extractionFailuresByReason.validation ?? 0) + 1;
          source.nextCheckAt = nextCheckAt(source, now).toISOString();
          continue;
        }
        ...
        if (candidate.discoveryExternalId) {
          ...
          for (const opportunity of this.store.opportunities.values()) {
            ...
            if (!opportunity.alternateSourceIds.includes(candidate.sourceId))
              continue;
            opportunity.alternateSourceIds =
              opportunity.alternateSourceIds.filter(
            ...
            if (opportunity.conflicts.length === 0) {
              if (opportunity.fields.deadline.kind === "conflicting") {
                opportunity.fields.deadline.kind = opportunity.fields.deadline
                  .date
                  ? "exact"
                  : "unknown";
              }
        ...
        if (match.kind === "same-page") {
          ...
          if (changes.length > 0)
            report.opportunitiesUpdated.push(match.opportunity.id);
        ...
        report.processingFailures++;
        report.extractionFailures++;
        report.extractionFailuresByReason.extractor =
          (report.extractionFailuresByReason.extractor ?? 0) + 1;
        source.nextCheckAt = nextCheckAt(source, now).toISOString();
      }
    }
    ...
    const matches = matchProfiles(
      this.store.radarProfiles.values(),
      this.store.opportunities.values(),
      now,
    );
    report.alerts.push(
  ...
  private createOpportunity(
    ...
    const fields: OpportunityFields = {
      ...
      // A directory URL is discovery evidence, never the public destination.
      // Only an extracted first-party URL may become the official CTA.
      guidelinesUrl: candidate.officialUrl,
  ...
  private applyUpdate(
    ...
    if (
      !("deadline" in overrides) &&
      candidate.deadline.date &&
      candidate.deadline.date !== f.deadline.date
    ) {
    ...
    if (
      !("fee" in overrides) &&
      candidate.fee.disclosed &&
      candidate.fee.amountCents !== f.fee.amountCents
    ) {
      record(
        "fee-changed",
        "fee",
    ...
    if (
      ...
      JSON.stringify(candidate.genres) !== JSON.stringify(f.genres)
    ) {
      record(
        "genres-changed",
        "genres",
    ...
    if (
      ...
      candidate.submissionUrl !== f.submissionUrl
    ) {
      record(
    ...
    if (nowClosed && !opp.lastClosedSignal) {
      ...
      !nowClosed &&
      opp.lastClosedSignal &&
      candidate.openSignals.length > 0
    ) {
    ...
    if (candidate.organizationName && !("organizationName" in overrides))
      f.organizationName = candidate.organizationName;
    if (candidate.prize) f.prize = candidate.prize;
    if (candidate.requiredMaterials.length > 0)
      f.requiredMaterials = candidate.requiredMaterials;
    if (candidate.simultaneousAllowed !== undefined)
      f.simultaneousAllowed = candidate.simultaneousAllowed;
  ...
  private mergeDuplicate(
    ...
    if (
      ...
      !(
        canonical.organizationOverrides &&
        "deadline" in canonical.organizationOverrides
      )
    ) {
    ...
    if (!f.submissionUrl && candidate.submissionUrl)
      f.submissionUrl = candidate.submissionUrl;
    if (!f.guidelinesUrl && candidate.officialUrl)
      f.guidelinesUrl = candidate.officialUrl;
    if (!f.prize && candidate.prize) f.prize = candidate.prize;
    if (f.genres.length === 0) f.genres = candidate.genres;
    if (
      !f.taxonomyAssignments?.length &&
      candidate.taxonomyAssignments?.length
    ) {
      f.taxonomyAssignments = candidate.taxonomyAssignments;
    }
  ...
  updateClaimedListing(
    opportunityId: string,
    organizationId: string,
    overrides: Partial<OpportunityFields>,
  ) {
    const opp = applyOrganizationOverride(
      this.ctx,
      opportunityId,
      organizationId,
      overrides,
  ...
  stats(): RadarStats {
    const opps = [...this.store.opportunities.values()].filter(
      (o) => !o.duplicateOfId,
    );
    ...
    const openTasks = [...this.store.verificationTasks.values()].filter(
      (t) => t.status === "open",
    ).length;
    const dupCount =
      [...this.store.opportunities.values()].filter((o) => o.duplicateOfId)
        .length + opps.reduce((n, o) => n + o.alternateSourceIds.length, 0);
    return {
      opportunitiesDiscovered: opps.length,
      opportunitiesOpen: opps.filter((o) =>
        ["open", "closing-soon", "deadline-extended", "opening-soon"].includes(
          o.status,
        ),
      ).length,
      opportunitiesClaimed: opps.filter((o) => o.claimedByOrganizationId)
        .length,
      staleListings: opps.filter((o) => o.status === "uncertain").length,
      duplicateRate:
        opps.length === 0 ? 0 : dupCount / (opps.length + dupCount),
