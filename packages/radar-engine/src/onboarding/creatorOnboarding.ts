...
import {
  ...
  type WriterGenre,
  type WriterFormat,
} from "../recommendation/writerMatchingEngine.js";

const DEFAULT_DISCIPLINE_OPPORTUNITY_TYPES: Record<
  CreativePracticeFamily,
  OpportunityType[]
> = {
  ...
  design: ["contest", "award", "grant", "residency", "commission", "rfp"],
...
export async function processCreatorOnboarding(
  ...
  const secondaryPractices = input.secondaryPractices.filter(
    (p) => p !== primaryPractice,
  );
  ...
  const disciplines: string[] = allPractices.map(
    (p) => PRACTICE_FAMILY_LABELS[p],
  );
  const genres: string[] = [];
  ...
  const configuredTypes: OpportunityType[] =
    ...
    input.preferences.opportunityTypes.length > 0
      ? input.preferences.opportunityTypes
      : DEFAULT_DISCIPLINE_OPPORTUNITY_TYPES[primaryPractice] || [
          "grant",
          "residency",
          "open-call",
          "award",
  ...
  const maxFeeCents =
    input.preferences.maxFeeCents ??
    (feePreference === "low_fee_acceptable" ? 500 : undefined);
  ...
  const simultaneousRequired =
    input.facets.writing?.simultaneousRequired ?? true;
  ...
  const writerProfile: WriterMatchingProfile = {
    userId,
    primaryGenres:
      writerGenres.length > 0
        ? writerGenres
        : ["Poetry", "Fiction", "Creative Nonfiction", "Essays"],
    subgenres: input.facets.writing?.subgenres ?? [],
    preferredFormats: (input.facets.writing
      ?.preferredFormats as WriterFormat[]) ?? ["Print", "Digital"],
    feePreference,
    maxFeeCents,
