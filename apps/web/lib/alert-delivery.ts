...
export async function deliverPendingAlertEmails(engine: RadarEngine, now = new Date()): Promise<AlertDeliveryReport> {
  ...
  for (const [userId, alerts] of byUser) {
    ...
    const effectKey = `alert-digest:${userId}:${createHash('sha256').update(alerts.map((alert) => alert.id).sort().join('|')).digest('hex').slice(0, 24)}`;
    ...
    try {
      ...
      if (effect && process.env.DATABASE_URL) await completePlatformMessageEffect({ connectionString: process.env.DATABASE_URL, effectId: effect.effectId, attemptNumber: effect.attemptNumber, status: 'sent', providerMessageId: result.data?.id });
      const sentAt = now.toISOString();
      for (const alert of alerts) alert.emailSentAt = sentAt;
      recipients += 1; sentAlerts += alerts.length;
    } catch (error) {
