 ...
 */
export default async function SubmissionsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-8">
      ...
      <form className="mt-5 flex flex-wrap items-end gap-2 rounded-lg border border-border bg-card p-3" method="get">
        ...
        {(query || statusFilter) && <Link href="/submissions" className="min-h-11 rounded-md border border-border px-4 py-2 text-sm text-foreground">Clear</Link>}
      </form>
