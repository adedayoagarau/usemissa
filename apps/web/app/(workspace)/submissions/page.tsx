...
export default async function SubmissionsPage({
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-8">
      ...
      <section aria-label="Submission insights" className="mt-4 rounded-lg border border-border bg-card p-4">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="flex flex-wrap gap-6 text-sm">
            <span>
              <b className="block font-mono text-xl">{reporting.submissions}</b>
              submissions
            </span>
            <span>
              <b className="block font-mono text-xl">{Math.round(reporting.conversionRate * 100)}%</b>
              acceptance rate
            </span>
            <span>
              <b className="block font-mono text-xl">{reporting.medianDaysToDecision ?? '—'}</b>
              median days to decision
            </span>
          </div>
          <a className="min-h-11 rounded-md border border-border px-3 py-2 text-sm hover:bg-muted" href={`/api/orgs/${organizationId}/insights/export`}>
            Export CSV
          </a>
        </div>
      </section>
      ...
      {submissions.length === 0 && <p className="mt-6 text-muted-foreground">No submissions yet. New submissions will appear here after an opportunity is published.</p>}
