...
export default async function WorkspaceReviewsPage({ searchParams }: { searchParams: Promise<{ organizationId?: string }> }) {
  ...
  const assignments = [...access.workspace.store.reviewAssignments.values()].filter((assignment) => {
    const submission = access.workspace.store.submissions.get(assignment.submissionId);
    return submission && submissions.some((item) => item.id === submission.id);
  });
  return (
    <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
      <header>
        <p className="text-xs font-medium tracking-[0.14em] text-muted-foreground uppercase">{access.organizationName}</p>
        <h1 className="mt-2 font-heading text-3xl font-medium tracking-tight text-foreground">Reviews</h1>
        ...
        <ul className="divide-y divide-border">
          {submissions.map((submission) => (
            <li key={submission.id} className="flex flex-wrap items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
              <div>
                <p className="text-sm font-medium text-foreground">{submission.openCallTitle}</p>
                <p className="mt-1 font-mono text-[11px] text-muted-foreground">
                  {submission.id} · {submission.submittedAt}
                </p>
              </div>
              <span className="text-xs text-muted-foreground">
                {assignments.filter((assignment) => assignment.submissionId === submission.id).length} assignment
                {assignments.filter((assignment) => assignment.submissionId === submission.id).length === 1 ? '' : 's'}
              </span>
            </li>
          ))}
          {submissions.length === 0 && <li className="py-8 text-sm text-muted-foreground">No submissions are currently in review.</li>}
        </ul>
      </section>
      <p className="mt-4 text-xs text-muted-foreground">
        <Link href="/reviewer" className="font-medium text-accent-deep underline decoration-accent-tint underline-offset-4">
          Open reviewer queue
        </Link>{' '}
        to see assigned submissions.
      </p>
    </main>
  );
}
