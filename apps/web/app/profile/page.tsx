...
import { ProfileProduct, type ProfileProductData, type ProfileSection } from '@/components/profile-product';
import { getSessionAccountFromToken, SESSION_COOKIE } from '@/lib/auth';
import { getEngine } from '@/lib/engine';

const PROFILE_SECTION_VALUES: readonly ProfileSection[] = ['overview', 'identity', 'preferences', 'privacy', 'integrations', 'searches', 'following', 'data'];

function sectionFrom(value: string | string[] | undefined): ProfileSection {
  const candidate = Array.isArray(value) ? value[0] : value;
  return PROFILE_SECTION_VALUES.includes(candidate as ProfileSection) ? candidate as ProfileSection : 'overview';
}
...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const returnPath = initialSection === 'overview' ? '/profile' : `/profile?section=${encodeURIComponent(initialSection)}`;
  const cookieStore = await cookies();
  const session = await getSessionAccountFromToken(cookieStore.get(SESSION_COOKIE)?.value);
  if (!session?.account.userId) redirect(`/login?next=${encodeURIComponent(returnPath)}`);
  ...
  return <div className="min-h-screen bg-white"><AppNav email={session.account.email} userId={session.account.userId} isAdmin={session.account.isAdmin} organizations={organizations} /><ProfileProduct initialSection={initialSection} initialProfile={profile} savedSearches={savedSearches} following={following} /></div>;
}
