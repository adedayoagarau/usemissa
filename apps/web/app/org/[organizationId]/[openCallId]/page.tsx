    ...
    <main id="main-content" className={styles.main}>
      ...
      <section id="application" className={styles.application} aria-labelledby="application-title">
        ...
        {!path ? <div className={styles.unavailable}><LockKeyhole aria-hidden="true" /><h3>Application form not published</h3><p>This Opportunity has no hosted submission path. No draft can be started here.</p></div> : !session ? <div className={styles.authGate}><LockKeyhole aria-hidden="true" /><p className={styles.eyebrow}>Your application is private</p><h3>Sign in before starting a draft</h3><p>Reading remains public. Missa creates or restores private answers and files only after you authenticate and deliberately use the form.</p><div><Link href={`/login?next=${encodeURIComponent(returnPath)}`}>Log in and return</Link><Link href={`/signup?next=${encodeURIComponent(returnPath)}`}>Create Profile</Link></div></div> : <div className={styles.desk}>
          <aside className={styles.ledger} aria-label="Current application sections"><p>Application</p><ol><li data-state="available"><span><Check aria-hidden="true" /></span><div><strong>Readiness</strong><small>Known requirements below</small></div></li><li data-state="current"><span>2</span><div><strong>Works</strong><small>At least one titled Work</small></div></li><li><span>3</span><div><strong>Questions</strong><small>{path.fields.length} fields · {requiredQuestions} required</small></div></li><li><span>4</span><div><strong>Review</strong><small>Confirm the packet before sending</small></div></li></ol></aside>
          <section className={styles.editor} aria-label="Current hosted application form"><div className={styles.compatibility}><Info aria-hidden="true" /><div><strong>Current form with recovery safeguards</strong><p>Draft recovery, uploads, fee handoff, idempotent submission, packet review, upload progress and retry, deadline-race handling, and the receipt path are in place. Form-version comparison and hosted provider checks remain promotion gates.</p></div></div><SubmitForm pathId={path.id} categories={path.categories} fields={path.fields} feeCents={path.feeCents} /></section>
          <aside className={styles.requirements} aria-label="Opportunity and draft context"><section><p className={styles.eyebrow}>Opportunity</p><h3>{call.title}</h3><dl><div><dt>Deadline</dt><dd>{publicDeadlineLabel(opportunity)}</dd></div><div><dt>Fee</dt><dd>{path.feeCents ? `$${(path.feeCents / 100).toFixed(2)} USD` : publicFeeLabel(opportunity)}</dd></div><div><dt>Categories</dt><dd>{path.categories.length || 'Not limited'}</dd></div></dl></section><section><p className={styles.eyebrow}>Current requirements</p><ul><li>At least one titled Work</li><li>{path.fields.length} Organization {path.fields.length === 1 ? 'field' : 'fields'}</li><li>Files up to 25 MB each</li><li>Private until final submission</li></ul></section><section><p className={styles.eyebrow}>Draft boundary</p><p>A local copy and current server draft may be restored. If the server changed, Missa keeps your answers here and asks you to review before saving again. Form-version comparison remains a promotion gate.</p></section></aside>
        </div>}
      </section>
    </main>
  </>;
