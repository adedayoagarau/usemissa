...
export default async function PublicHandlePage({params}:{params:Promise<{handle:string}>}) {
 ...
 if(!resolved||resolved.state!=='claimed'||resolved.subjectType!=='user')notFound();
 const repo=getCreatorProfileRepository();
 const parsed=portfolioSchema.safeParse(await repo?.publicPortfolio(resolved.subjectId));
 if(!parsed.success)notFound();
 if(resolved.resolution==='alias'||raw!==`@${resolved.handleKey}`)permanentRedirect(`/@${resolved.handleKey}`);
 return <PublicSiteShell><CreatorPortfolioStudio publicData={{...parsed.data,handle:resolved.handleKey}} /></PublicSiteShell>;
}
