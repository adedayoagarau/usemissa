...
import { discoveryContentLastModified, discoveryContentLastModifiedLabel, discoveryGuides } from '@/lib/discoveryGuides';
...
export default function GuidesPage() {
  return (
    <main className="min-h-screen bg-background">
      ...
      <JsonLd data={{ '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'Opportunity guides for creators', url: absoluteUrl('/guides'), dateModified: discoveryContentLastModified.toISOString(), mainEntity: { '@type': 'ItemList', itemListElement: discoveryGuides.map((guide, index) => ({ '@type': 'ListItem', position: index + 1, name: guide.title, url: absoluteUrl(`/guides/${guide.slug}`) })) } }} />
