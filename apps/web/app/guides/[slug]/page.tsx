...
import { discoveryContentLastModified, discoveryContentLastModifiedLabel, discoveryGuide, discoveryGuides } from '@/lib/discoveryGuides';
...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <JsonLd data={{ '@context': 'https://schema.org', '@type': 'WebPage', name: guide.title, description: guide.description, url: absoluteUrl(`/guides/${guide.slug}`), dateModified: discoveryContentLastModified.toISOString(), isPartOf: { '@type': 'WebSite', name: 'Missa', url: absoluteUrl('/') }, mainEntity: { '@type': 'ItemList', numberOfItems: items.length, itemListElement: items.map((item, index) => ({ '@type': 'ListItem', position: index + 1, name: item.title, url: absoluteUrl(`/discover/opportunities/${item.slug}`) })) } }} />
      ...
      <article className="mx-auto max-w-5xl px-6 py-14">
        ...
        <header className="mt-8 max-w-3xl"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Opportunity guide</p><h1 className="mt-3 font-heading text-4xl font-medium tracking-[-0.04em] sm:text-6xl">{guide.title}</h1><p className="mt-5 text-base leading-7 text-muted-foreground">{guide.description}</p><p className="mt-3 text-xs text-muted-foreground"><time dateTime={discoveryContentLastModified.toISOString()}>Reviewed {discoveryContentLastModifiedLabel}</time></p></header>
        ...
        <section className="mt-12 max-w-3xl" aria-labelledby="faq-heading"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Questions creators ask</p><h2 id="faq-heading" className="mt-2 font-heading text-3xl font-medium">A clearer way to compare the details.</h2><div className="mt-6 space-y-4">{guide.faqs.map((faq) => <article key={faq.question} className="rounded-xl border border-border bg-card p-6"><h3 className="font-heading text-xl font-medium">{faq.question}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{faq.answer}</p></article>)}</div></section>
