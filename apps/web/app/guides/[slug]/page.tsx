  ...
  return <PublicSiteShell current="Guides"><main id="main-content" className={styles.main}>
    ...
    <div className={styles.articleLayout}>
      ...
      <aside className={styles.related} aria-labelledby="related-heading">
        ...
        {items.length ? <div className={styles.relatedList}>{items.slice(0, 4).map((item) => <article key={item.id}><h3>{item.title}</h3><p>{item.organizationName ?? 'Organization not listed'} · {deadlineLabel(item)} · {feeLabel(item)}</p><Link href={`/opportunities/${item.slug}`}>Open Opportunity <ArrowRight aria-hidden="true" /></Link><a href={item.source.url} target="_blank" rel="noreferrer">Official source <ExternalLink aria-hidden="true" /></a></article>)}</div> : <><p>{unavailable ? 'Related Opportunities are temporarily unavailable. The Guide remains readable.' : 'Missa has no matching published records in this collection right now. That does not mean no such Opportunities exist.'}</p><Link href="/opportunities">Browse all Opportunities <ArrowRight aria-hidden="true" /></Link></>}
      </aside>
    </div>
