...
export async function GET(
  ...
  if (repository)
    return NextResponse.json(await repository.follows(auth.session.account.id));
  const engine = await getEngine();
  const follows = engine.store.follows.filter((f) => f.userId === id);
  const withNames = follows.map((f) => ({
    organizationId: f.organizationId,
    organizationName:
      engine.store.organizations.get(f.organizationId)?.name ??
      f.organizationId,
...
export async function POST(
  ...
  if (repository) {
    try {
      const receipt = await new CreatorFollowingRepository().follow(
        creatorCommandEnvelope(
          auth.session.account.id,
          "organization-follow.create",
      ...
      return NextResponse.json(
        { ok: true, revision: receipt.revision, idempotent: receipt.replayed },
        { status: receipt.replayed ? 200 : 201 },
      );
    } catch (error) {
      if (error instanceof CreatorCommandValidationError)
        return NextResponse.json({ error: error.message }, { status: 400 });
      if (
        error instanceof CreatorConflictError ||
        error instanceof FollowingValidationError
      )
        return NextResponse.json(
      ...
      return NextResponse.json(
        { error: "We could not follow that Organization." },
        { status: 500 },
      );
    }
  }
