...
import { CreatorFollowingRepository } from "@/lib/creator-following";
...
export async function DELETE(
  request: Request,
  { params }: { params: Promise<{ id: string; organizationId: string }> },
) {
  ...
  if (repository) {
    ...
    try {
      const receipt = await new CreatorFollowingRepository().unfollow(
        creatorCommandEnvelope(
          auth.session.account.id,
          "organization-follow.delete",
  ...
  engine.store.follows = engine.store.follows.filter(
    (f) => !(f.userId === id && f.organizationId === organizationId),
  );
