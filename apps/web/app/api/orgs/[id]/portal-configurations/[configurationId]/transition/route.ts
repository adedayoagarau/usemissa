...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; configurationId: string }> }) {
  ...
  try {
    ...
    const payload = { id: configurationId, status: status.data };
    const command = workspaceCommandEnvelope(request, { actorAccountId: access.access.session.account.id, organizationId: id, commandType: 'portal_configuration.transition', payload, expectedRevision: body.expectedRevision });
    const transitioned = await workspace.transitionPortalConfiguration(command, configurationId, status.data);
    return NextResponse.json({ id: transitioned.resourceId, revision: transitioned.revision, receiptId: transitioned.receiptId, idempotent: transitioned.replayed });
  } catch (error) {
