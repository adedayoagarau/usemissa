...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; openCallId: string; versionId: string }> }) {
  ...
  try {
    ...
    const command = workspaceCommandEnvelope(request, { actorAccountId: access.access.session.account.id, organizationId: id, commandType: 'review_workflow_version.update', payload: parsed.data, expectedRevision: body.expectedRevision });
    const updated = await workspace.updateReviewWorkflowVersion(command, versionId, parsed.data);
