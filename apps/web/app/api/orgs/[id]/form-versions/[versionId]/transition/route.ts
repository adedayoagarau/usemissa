...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; versionId: string }> }) {
  ...
  try {
    ...
    const command = workspaceCommandEnvelope(request, { actorAccountId: access.access.session.account.id, organizationId: id, commandType: `form_version.${status.data}`, payload: { versionId, status: status.data }, expectedRevision: body.expectedRevision });
    const changed = await workspace.transitionFormVersion(command, versionId, status.data);
