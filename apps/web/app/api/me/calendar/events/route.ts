...
const eventId = (key: string) =>
  `calendar_${createHash("sha256").update(`calendar:${key}`).digest("hex").slice(0, 32)}`;
...
export async function POST(request: Request) {
  ...
  if (!key || !body || typeof body !== "object")
    return json(
      { error: "Event details and Idempotency-Key are required." },
      400,
    );
  try {
    const id = eventId(key),
      receipt = await repository.createEvent(
        creatorCommandEnvelope(
          session.account.id,
...
function calendarError(error: unknown) {
  return error instanceof CreatorCalendarError
    ? json(
        { error: error.message },
        error.message.includes("changed") ? 409 : 400,
      )
    : json({ error: "We could not update this event." }, 500);
}
