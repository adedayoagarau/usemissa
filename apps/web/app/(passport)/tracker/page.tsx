...
function safeView(value: string): TrackerProductView {
  return trackerViews.has(value as TrackerProductView) ? value as TrackerProductView : 'active';
}
...
export default async function TrackerPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const tracker = radar.getTracker(userId);
  const initialItems: TrackerProductItem[] = Object.values(tracker.pipeline)
    .flat()
    .map((item) => ({
  ...
  const hostedSubmissions: TrackerHostedSubmission[] = [...workspace.store.submissions.values()]
    ...
    .map((submission) => {
      ...
      return {
        ...
        organizationName: organization?.name ?? entity?.name ?? 'Organization not listed',
        ...
        works: workspace.worksForSubmission(submission.id).map((work) => ({
          id: work.id,
          title: work.title,
          outcome: decisions.find((decision) => decision.workId === work.id)?.outcome,
        })),
        paymentStatus: submission.paymentStatus,
      };
    });

  const works = radar.library(userId).works.map((work) => ({ id: work.id, title: work.title }));
