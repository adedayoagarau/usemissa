...
export default async function OrganizationSubmissionDossierPage({ params, searchParams }: { params: Promise<{ organizationId: string; submissionId: string }>; searchParams: Promise<{ section?: string }> }) {
  ...
  const relationalWorkspace = workspaceRelationalAuthorityEnabled() ? await getRelationalWorkspace() : undefined;
  const workspace = relationalWorkspace ? undefined : await getWorkspaceEngine();
  const submission = relationalWorkspace
    ? (await relationalWorkspace.submissionsForOrganization(organizationId)).find((item) => item.id === submissionId)
    : workspace!.submissionsForOrganization(organizationId).find((item) => item.id === submissionId);
  ...
  const path = relationalWorkspace ? await relationalWorkspace.publicSubmissionPath(submission.submissionPathId) : workspace!.store.submissionPaths.get(submission.submissionPathId);
  const works = 'works' in submission ? submission.works : workspace!.worksForSubmission(submissionId);
  const assignments = relationalWorkspace ? await relationalWorkspace.reviewAssignmentsForSubmission(organizationId, submissionId) : workspace!.reviewAssignmentsForSubmission(submissionId);
  const decisions = 'decisions' in submission ? submission.decisions : workspace!.decisionsForSubmission(organizationId, submissionId);
  const delivery = relationalWorkspace ? [] : workspace!.deliveryTasksForOrganization(organizationId).filter((task) => works.some((work) => work.id === task.workId));
  ...
  return <main id="organization-main" className={styles.main}><Link className={styles.back} href={`/organization/${encodeURIComponent(organizationId)}/submissions`}>← Submissions</Link><header className={styles.header}><div><p className={styles.eyebrow}>{submission.openCallTitle}</p><h1>{submitterProfile?.displayName || submitter?.displayName || submitter?.email || 'Submission dossier'}</h1><p>{submission.category || 'No category supplied'} · Received {new Intl.DateTimeFormat('en', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(submission.submittedAt))}</p></div><aside className={styles.nextAction}><span>Next safe action</span><strong>{next}</strong></aside></header><nav className={styles.tabs} aria-label="Submission dossier sections">{sections.map((item) => <Link key={item.id} href={`${base}?section=${item.id}`} aria-current={active === item.id ? 'page' : undefined}>{item.label}</Link>)}</nav>
    <section className={styles.content} aria-labelledby={`dossier-${active}`}>
      ...
      {active === 'reviews' ? <><header><h2 id="dossier-reviews">Reviews</h2><p>Assignments and completed recommendations remain separate from decisions.</p></header><div className={styles.list}>{assignments.map((assignment) => { const reviewerId = typeof assignment.reviewerAccountId === 'string' ? assignment.reviewerAccountId : undefined; const reviewer = reviewerId ? radar.store.accounts.get(reviewerId) : undefined; const assignmentId = assignment.id as string; const recommendation = 'recommendation' in assignment && assignment.recommendation && typeof assignment.recommendation === 'object' ? assignment.recommendation as { score?: number } : workspace ? workspace.recommendationForAssignment(assignmentId) : undefined; return <article className={styles.item} key={assignmentId}><div><h3>{reviewer?.displayName || reviewer?.email || 'Reviewer'}</h3><p>{recommendation ? `Recommendation recorded${recommendation.score === undefined ? '' : ` · Score ${recommendation.score}`}` : 'Recommendation not submitted'}</p></div><span>{assignment.completedAt ? 'Complete' : 'In progress'}</span></article>; })}{assignments.length ? null : <div className={styles.boundary}>No review assignments are recorded for this Submission.</div>}</div></> : null}
