...
const statusClass: Record<string, string> = {
  published: "border-success/30 bg-success-subtle text-success",
  recommended: "border-information/30 bg-information-subtle text-information",
  queued: "border-border bg-muted text-muted-foreground",
  processing: "border-information/30 bg-information-subtle text-information",
  needs_human: "border-warning/30 bg-warning-subtle text-warning",
  failed: "border-destructive/30 bg-destructive-subtle text-destructive",
  held: "border-border bg-muted text-foreground",
...
function State({ ok, label }: { ok: boolean; label: string }) {
  return <span className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs font-medium ${ok ? "border-success/30 bg-success-subtle text-success" : "border-warning/30 bg-warning-subtle text-warning"}`}>
    <span className={`size-2 rounded-full ${ok ? "bg-success" : "bg-warning"}`} aria-hidden="true" />{label}
  </span>;
}
...
function ReviewActions({ row }: { row: GaryReviewRow }) {
  ...
  async function act(action: GaryQueueAction) {
    ...
    try {
      ...
      setMessage(
        action === "publish"
          ? "Publication request accepted. Refreshing the queue does not confirm the public page is live."
          : action === "reject"
            ? "Rejection recorded. Refreshing the queue…"
            : `${action[0].toUpperCase()}${action.slice(1)} request accepted. Refreshing the queue…`,
      );
      router.refresh();
    } catch (error) {
  ...
  return <div>
    <div className="flex flex-wrap gap-2">
      {row.status !== "published" && <AdminConfirmationAction
        label="Publish"
        title={`Request publication for “${row.title}”?`}
        description="This records a publication request for Gary's publisher lane. It does not confirm that the public page is live. Review the evidence and current queue state before continuing."
        confirmLabel="Request publication"
        disabled={Boolean(busy)}
        working={busy === "publish"}
        workingLabel="Requesting…"
        evidence={[
          { label: "Job", value: row.id },
          { label: "Organizer", value: row.organizer },
          { label: "Current status", value: row.status.replaceAll("_", " ") },
          { label: "Deadline", value: row.deadline ?? "Not observed" },
          { label: "Review evidence", value: row.reasons.length > 0 ? row.reasons.join(" · ") : "No review reasons recorded" },
        ]}
      ...
      {!['published', 'rejected'].includes(row.status) && <AdminConfirmationAction
        ...
        description="This records a rejection for the named Gary review job and keeps it out of the publisher lane. It does not change the publisher's official source."
        confirmLabel="Reject review job"
        destructive
        disabled={Boolean(busy)}
        working={busy === "reject"}
...
export default function PlatformAdminGary({ area }: { area: AdminArea<GaryDashboardData> }) {
  ...
  return <div className="space-y-8">
    ...
    <section className="rounded-xl border border-border bg-card p-5" aria-labelledby="gary-readiness">
    ...
    <section className="space-y-4" aria-labelledby="gary-review-queue">
      ...
      {rows.length === 0 ? <div className="rounded-xl border border-dashed border-border p-8 text-center"><p className="font-medium">Nothing in this view</p><p className="mt-1 text-sm text-muted-foreground">Gary has no matching exceptions right now.</p></div> :
        <div className="space-y-3">{rows.map((row) => <article key={row.id} className="rounded-xl border border-border bg-card p-5">
          ...
          {row.lastError && <p className="mt-3 rounded-md bg-destructive-subtle px-3 py-2 text-xs text-destructive">{row.lastError}</p>}
