...
const VISIBLE_NAV_LINKS = process.env.NEXT_PUBLIC_MISSA_CHAT_ENABLED?.trim() === '1' ? NAV_LINKS : NAV_LINKS.filter((link) => link.href !== '/ask');
...
export function AppNav({ email, userId, isAdmin = false, organizations = [] }: { email: string; userId?: string; isAdmin?: boolean; organizations?: Array<{ id: string; name: string }> }) {
  ...
  function organizationPicker(className: string) {
    ...
    return <label className={`flex min-w-0 items-center gap-2 ${className}`}><span className="sr-only">Organization workspace</span><select aria-label="Organization workspace" value={organizations.some((organization) => organization.id === currentOrganizationId) ? currentOrganizationId! : organizations[0]?.id ?? ''} onChange={(event) => handleOrganizationChange(event.target.value)} className="min-h-11 min-w-0 max-w-52 rounded-md border border-border bg-white px-2.5 text-xs font-medium text-foreground outline-none focus-visible:ring-3 focus-visible:ring-ring/50">{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select></label>;
  }
  ...
  return (
    <header className="relative z-50 flex min-w-0 items-center gap-3 border-b border-border bg-white px-4 py-3 sm:px-6">
      ...
      <div className="hidden items-center gap-1 lg:flex">
        {userId && <Link href="/profile" aria-current={pathname === '/profile' ? 'page' : undefined} className={`rounded-md px-2.5 py-2 text-sm transition-colors hover:bg-muted ${pathname === '/profile' ? 'font-medium text-foreground' : 'text-muted-foreground'}`}>Profile</Link>}
        {organizations.length > 0 && <Link href={scopedHref('/workspace')} aria-current={pathname.startsWith('/workspace') ? 'page' : undefined} className={`rounded-md px-2.5 py-2 text-sm transition-colors hover:bg-muted ${pathname.startsWith('/workspace') ? 'font-medium text-foreground' : 'text-muted-foreground'}`}>Workspace</Link>}
      ...
      {mobileNavOpen && <nav aria-label="Mobile navigation" className="absolute inset-x-0 top-full border-b border-border bg-white p-2 shadow-lg lg:hidden">
        <div className="grid gap-1">
          ...
          {organizations.length > 0 && <Link href={scopedHref('/workspace')} onClick={() => setMobileNavOpen(false)} aria-current={pathname.startsWith('/workspace') ? 'page' : undefined} className={`flex min-h-11 items-center rounded-lg px-3 text-sm ${pathname.startsWith('/workspace') ? 'bg-accent-tint font-medium text-accent-deep' : 'text-foreground hover:bg-muted'}`}>Workspace</Link>}
