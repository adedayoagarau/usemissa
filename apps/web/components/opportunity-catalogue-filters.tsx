...
import { MISSA_TAXONOMY, taxonomyTermById, type TaxonomyFacetKey } from '@missa/taxonomy';
...
function FilterPanel({
  ...
  const practiceFamilies = useMemo(() => termsForFacet('practice-family'), []);
  ...
  function toggleValue(key: string, value: string, checked: boolean) {
    ...
    const retained = next.getAll(key).filter((candidate) => candidate !== value);
    next.delete(key);
  ...
  function setFacet(facet: TaxonomyFacetKey, value: string) {
    const next = new URLSearchParams(searchParams.toString());
    const retained = next.getAll('taxonomy').filter((termId) => taxonomyTermById(termId)?.facet !== facet);
  ...
  return (
    <form className={styles.panel} onSubmit={(event) => event.preventDefault()}>
      ...
      <label className={styles.selectField}>
        ...
        <select value={searchParams.get('location') ?? ''} onChange={(event) => update('location', event.target.value)}>
      ...
      <label className={styles.selectField}>
        <span>Deadline</span>
        <select value={searchParams.get('deadlineWithinDays') ?? ''} onChange={(event) => update('deadlineWithinDays', event.target.value)}>
      ...
      <label className={styles.selectField}>
        <span>Fee</span>
        <select value={searchParams.get('fee') ?? ''} onChange={(event) => update('fee', event.target.value)}>
      ...
      <details className={styles.more}>
        <summary>
          <span><strong>More category filters</strong><small>Keep each category separate.</small></span>
          <ChevronDown aria-hidden="true" />
        </summary>
        <div className={styles.moreFields}>
          {visibleFacets.filter((facet) => facet.key !== 'practice-family').map((facet) => {
            const selected = selectedTerms.find((termId) => taxonomyTermById(termId)?.facet === facet.key) ?? '';
            return (
              <label key={facet.key} className={styles.selectField}>
                <span>{facet.label}</span>
                <select value={selected} onChange={(event) => setFacet(facet.key, event.target.value)}>
                  <option value="">Any</option>
                  {termsForFacet(facet.key).map((term) => <option key={term.id} value={term.id}>{term.preferredLabel}</option>)}
                </select>
              </label>
            );
          })}
      ...
      <div className={styles.status} aria-live="polite">
        {pending ? <><RefreshCw className="animate-spin" aria-hidden="true" />Updating results…</> : null}
      </div>
      <Button type="button" className={styles.doneButton} onClick={onDone}>Show opportunities</Button>
    </form>
  );
}
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="bottom" className={styles.sheet}>
          <SheetHeader>
            ...
            <SheetDescription>Use the details that matter for this search.</SheetDescription>
          </SheetHeader>
          <div className={styles.sheetBody}>
            <FilterPanel locations={locations} onDone={() => setSheetOpen(false)} />
          </div>
        </SheetContent>
      </Sheet>
    </div>
