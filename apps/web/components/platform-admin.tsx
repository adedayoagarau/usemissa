...
export function WarningList({ warnings }: { warnings: string[] }) {
  ...
  return <section aria-label="Admin data warnings" className="rounded-lg border border-warning/30 bg-warning-subtle px-4 py-3 text-sm text-warning"><p className="font-medium">Read-model caveats</p><ul className="mt-1 list-disc space-y-1 pl-5">{warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul></section>;
}

export function MetricCard({ label, value, detail, href }: { label: string; value: ReactNode; detail?: string; href?: string }) {
  const card = <div className="rounded-xl border border-border bg-card p-4 shadow-sm"><p className="text-xs font-medium text-muted-foreground">{label}</p><p className="mt-2 font-mono text-2xl tabular-nums text-foreground">{value}</p>{detail && <p className="mt-1 text-sm leading-5 text-muted-foreground">{detail}</p>}</div>;
...
export function QueueCard({ label, value, detail, href, tone = 'neutral' }: { label: string; value: number; detail: string; href: string; tone?: 'neutral' | 'warning' }) {
  return <Link href={href} className={`block rounded-xl border p-4 transition-shadow hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary ${tone === 'warning' ? 'border-warning/30 bg-warning-subtle/40' : 'border-border bg-card'}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium text-foreground">{label}</p><p className="mt-1 text-sm leading-5 text-muted-foreground">{detail}</p></div><span className="font-mono text-2xl tabular-nums text-foreground">{value}</span></div><span className="mt-3 inline-block text-sm font-medium text-accent-deep">Open queue →</span></Link>;
}
...
export function SourceHealthTable({ rows }: { rows: SourceHealthRow[] }) {
  ...
  return <div className="rounded-xl border border-border bg-card"><ul className="divide-y divide-border md:hidden" aria-label="Source health with attempt, fetch, process, and freshness distinctions">{rows.map((row) => <li key={row.id} className="space-y-4 px-4 py-5"><div><p className="font-semibold text-foreground">{row.name}</p><p className="mt-1 break-all font-mono text-xs text-muted-foreground">{row.id}</p></div><dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm"><div><dt className="text-xs font-medium text-muted-foreground">Attempted</dt><dd className="mt-1 text-foreground"><StatusText value={row.attempted} /></dd></div><div><dt className="text-xs font-medium text-muted-foreground">Successful fetch</dt><dd className="mt-1 text-foreground"><StatusText value={row.successfulFetch} /></dd></div><div><dt className="text-xs font-medium text-muted-foreground">Processed</dt><dd className="mt-1 text-foreground"><StatusText value={row.processed} /></dd></div><div><dt className="text-xs font-medium text-muted-foreground">Freshness</dt><dd className={`mt-1 break-words font-mono text-xs ${row.stale ? 'text-warning' : 'text-foreground'}`}>{row.stale ? 'stale · ' : ''}{row.freshness}</dd></div><div className="col-span-2"><dt className="text-xs font-medium text-muted-foreground">Fetch / processing failures</dt><dd className="mt-1 font-mono text-sm text-foreground">{row.consecutiveFailures} / {row.consecutiveProcessingFailures}</dd></div></dl></li>)}</ul><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] border-collapse text-left text-sm"><caption className="sr-only">Source health with attempt, fetch, process, and freshness distinctions</caption><thead className="border-b border-border bg-muted/40 text-xs text-muted-foreground"><tr><th scope="col" className="px-4 py-3 font-medium">Source</th><th scope="col" className="px-4 py-3 font-medium">Attempted</th><th scope="col" className="px-4 py-3 font-medium">Successful fetch</th><th scope="col" className="px-4 py-3 font-medium">Processed</th><th scope="col" className="px-4 py-3 font-medium">Freshness</th><th scope="col" className="px-4 py-3 font-medium">Failures</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-b border-border last:border-0 hover:bg-muted/30"><th scope="row" className="max-w-[260px] px-4 py-3 font-medium text-foreground"><span className="block truncate">{row.name}</span><span className="mt-0.5 block truncate font-mono text-[11px] font-normal text-muted-foreground">{row.id}</span></th><td className="px-4 py-3"><StatusText value={row.attempted} /></td><td className="px-4 py-3"><StatusText value={row.successfulFetch} /></td><td className="px-4 py-3"><StatusText value={row.processed} /></td><td className={`px-4 py-3 font-mono text-xs ${row.stale ? 'text-warning' : 'text-muted-foreground'}`}>{row.stale ? 'stale · ' : ''}{row.freshness}</td><td className="px-4 py-3 font-mono text-xs text-muted-foreground">{row.consecutiveFailures}/{row.consecutiveProcessingFailures}</td></tr>)}</tbody></table></div></div>;
}
...
export function LifecycleTable({ counts, label = 'Lifecycle' }: { counts: Record<string, number>; label?: string }) {
  ...
  return <div className="overflow-hidden rounded-xl border border-border bg-card"><table className="w-full text-left text-sm"><caption className="sr-only">{label} counts</caption><thead className="border-b border-border bg-muted/40 text-xs text-muted-foreground"><tr><th scope="col" className="px-4 py-3 font-medium">Status</th><th scope="col" className="px-4 py-3 text-right font-medium">Count</th></tr></thead><tbody>{entries.map(([status, count]) => <tr key={status} className="border-b border-border last:border-0"><th scope="row" className="px-4 py-3 font-medium text-foreground">{status.replaceAll('-', ' ')}</th><td className="px-4 py-3 text-right font-mono tabular-nums text-foreground">{count}</td></tr>)}</tbody></table></div>;
}

export function Pipeline({ stages }: { stages: PlatformAdminOperationsData['pipeline'] }) {
  return <ol className="grid gap-2 sm:grid-cols-3 lg:grid-cols-6">{stages.map((stage) => <li key={stage.stage} className="rounded-lg border border-border bg-card p-3"><p className="text-xs font-medium capitalize text-muted-foreground">{stage.stage}</p><p className="mt-1 font-mono text-xl tabular-nums text-foreground">{stage.count}</p><p className="mt-1 text-[11px] leading-4 text-muted-foreground">{stage.source}</p></li>)}</ol>;
}
...
export function WorkerStatus({ worker }: { worker: PlatformAdminOperationsData['worker'] }) {
  ...
  const statusStyle = worker.status === 'failed' ? 'border-destructive/30 bg-destructive-subtle text-destructive' : worker.status === 'stale' ? 'border-warning/30 bg-warning-subtle text-warning' : worker.status === 'unknown' ? 'border-border bg-muted text-muted-foreground' : worker.status === 'running' ? 'border-success/30 bg-success-subtle text-success' : 'border-border bg-card text-muted-foreground';
  return <div className="rounded-xl border border-border bg-card p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-medium text-foreground">Worker/system status</p><p className="mt-1 text-sm text-muted-foreground">{worker.caveat}</p></div><span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${statusStyle}`}>{statusLabel}</span></div><dl className="mt-4 grid grid-cols-3 gap-3 text-xs"><div><dt className="text-muted-foreground">Running lanes</dt><dd className="mt-1 font-mono text-base">{worker.running}</dd></div><div><dt className="text-muted-foreground">Failed lanes</dt><dd className="mt-1 font-mono text-base">{worker.failed}</dd></div><div><dt className="text-muted-foreground">Completed runs</dt><dd className="mt-1 font-mono text-base">{worker.completed}</dd></div></dl>{worker.latestAt && <p className="mt-3 font-mono text-xs text-muted-foreground">Latest {worker.latestKind ?? 'run'} · {worker.latestStatus ?? 'unknown'} · {worker.latestAt}</p>}</div>;
}
...
export function WorkerLaneTable({ lanes }: { lanes: PlatformAdminOperationsData['worker']['lanes'] }) {
  ...
  return <div className="overflow-x-auto rounded-xl border border-border bg-card"><table className="w-full min-w-[760px] text-left text-sm"><caption className="sr-only">Durable worker lane heartbeat status</caption><thead className="border-b border-border bg-muted/40 text-xs text-muted-foreground"><tr><th scope="col" className="px-4 py-3 font-medium">Lane</th><th scope="col" className="px-4 py-3 font-medium">Status</th><th scope="col" className="px-4 py-3 font-medium">Last heartbeat</th><th scope="col" className="px-4 py-3 font-medium">Started</th><th scope="col" className="px-4 py-3 font-medium">Error</th></tr></thead><tbody>{lanes.map((lane) => <tr key={lane.workerKind} className="border-b border-border last:border-0"><th scope="row" className="px-4 py-3 font-medium text-foreground">{lane.workerKind}</th><td className={`px-4 py-3 text-xs font-medium ${lane.status === 'running' ? 'text-success' : lane.status === 'stale' || lane.status === 'failed' ? 'text-warning' : 'text-muted-foreground'}`}>{lane.status}</td><td className="px-4 py-3 font-mono text-xs text-muted-foreground">{lane.lastHeartbeatAt ?? 'not observed'}</td><td className="px-4 py-3 font-mono text-xs text-muted-foreground">{lane.startedAt ?? 'not observed'}</td><td className="max-w-[280px] truncate px-4 py-3 text-xs text-destructive">{lane.error ?? '—'}</td></tr>)}</tbody></table></div>;
}
...
export function DurableTableList({ data }: { data: PlatformAdminSystemData }) {
  ...
  return <ul className="divide-y divide-border rounded-xl border border-border bg-card text-sm">{data.durableTables.map((table) => <li key={table.name} className="flex items-center justify-between gap-3 px-4 py-3"><span className="font-mono text-xs text-foreground">{table.name}</span><MaturityBadge maturity={table.status === 'deployed' ? 'target-schema' : 'unavailable'} /></li>)}</ul>;
}
