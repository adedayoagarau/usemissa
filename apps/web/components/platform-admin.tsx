...
export function WorkerStatus({ worker }: { worker: PlatformAdminOperationsData['worker'] }) {
  ...
  return <div className="rounded-xl border border-border bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-medium text-foreground">Worker/system status</p><p className="mt-1 text-xs text-muted-foreground">{worker.caveat}</p></div><span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${worker.status === 'failed' ? 'border-red-200 bg-red-50 text-red-700' : worker.status === 'unknown' ? 'border-border bg-muted text-muted-foreground' : worker.status === 'running' ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-green-200 bg-green-50 text-green-700'}`}>{statusLabel}</span></div><dl className="mt-4 grid grid-cols-3 gap-3 text-xs"><div><dt className="text-muted-foreground">Running</dt><dd className="mt-1 font-mono text-base">{worker.running}</dd></div><div><dt className="text-muted-foreground">Failed</dt><dd className="mt-1 font-mono text-base">{worker.failed}</dd></div><div><dt className="text-muted-foreground">Completed</dt><dd className="mt-1 font-mono text-base">{worker.completed}</dd></div></dl>{worker.latestAt && <p className="mt-3 font-mono text-[11px] text-muted-foreground">Latest {worker.latestKind ?? 'run'} · {worker.latestStatus ?? 'unknown'} · {worker.latestAt}</p>}</div>;
}
