...
const sections: Array<{ id: Section; label: string }> = [
  { id: 'basics', label: 'Basics' }, { id: 'guidelines', label: 'Guidelines' }, { id: 'practice', label: 'Practice rules' }, { id: 'eligibility', label: 'Eligibility' }, { id: 'place', label: 'Place' }, { id: 'dates', label: 'Dates' }, { id: 'fees', label: 'Fees and terms' }, { id: 'form', label: 'Submission form' }, { id: 'review', label: 'Review and publish' },
];
...
export function OrganizationOpportunityEditor({ organizationId, opportunity, teamName, programName, canEdit, form }: { organizationId: string; opportunity: Opportunity; teamName: string; programName: string; canEdit: boolean; form?: { categories: string[]; fieldCount: number; practiceRuleCount: number; feeCents?: number } }) {
  ...
  return <div className={styles.editor}><nav className={styles.sections} aria-label="Opportunity builder sections">{sections.map((item) => <button key={item.id} type="button" aria-current={section === item.id ? 'step' : undefined} onClick={() => { setSection(item.id); setMessage(''); }}>{item.label}</button>)}</nav><p className="sr-only" role="status" aria-live="polite">{message}</p>
    <section className={styles.panel} aria-labelledby={`opportunity-${section}`}>
      ...
      {section === 'practice' ? <><header className={styles.panelHeader}><p>Opportunity builder · Practice rules</p><h2 id="opportunity-practice">Practice rules</h2><p>Accepted creative work remains separate from eligibility, place, type, and application materials.</p></header><div className={styles.boundary}><h3>{form?.practiceRuleCount ?? 0} saved practice rules</h3><p>The compatibility form can store canonical rule assignments, but this new Organization surface does not yet provide conflict-safe editing across all 12 facets.</p></div></> : null}
      ...
      {section === 'review' ? <><header className={styles.panelHeader}><p>Opportunity builder · Review and publish</p><h2 id="opportunity-review">Review and publish</h2><p>Publishing is a consequential state transition, not a lifecycle toggle.</p></header><div className={styles.boundary}><h3>{opportunity.status === 'draft' ? 'Publication is blocked in this screen' : `Current lifecycle: ${opportunity.status}`}</h3><p>The new Organization product cannot yet prove readiness for all required public facts and the applicant flow. It therefore does not expose publish, close, or reopen controls.</p><ul><li>Eligibility, place, dates, and full commercial terms need dedicated records.</li><li>Practice-rule conflicts and deprecated terms need typed validation.</li><li>The real applicant flow needs a safe preview before publication.</li></ul></div></> : null}
    </section>
  </div>;
}
