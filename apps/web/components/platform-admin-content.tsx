...
export default function PlatformAdminContent({ area }: { area: AdminArea<PlatformAdminContentData> }) {
  ...
  const [pendingReview, setPendingReview] = useState<{ jobId: string; decision: 'approved' | 'blocked' }>();
  const rows = useMemo(() => {
  ...
  function resolveReview(jobId: string, decision: 'approved' | 'blocked') {
    setPendingReview({ jobId, decision });
  }

  async function commitReview(jobId: string, decision: 'approved' | 'blocked') {
    ...
    try {
      ...
      setReviewMessage(
        decision === 'approved'
          ? 'Approval recorded; a publisher-lane handoff was requested. This does not confirm publication.'
          : 'Block decision recorded; the generated projection remains out of public reads.',
      );
    } catch {
  ...
  const pendingReviewRow = pendingReview
    ? queueRows.find((row) => row.jobId === pendingReview.jobId)
    : undefined;

  return <div className="space-y-8">
    <div><p className="text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">Platform Admin · Product</p><h1 className="mt-2 font-sans text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">Content</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">One registry for the content Missa observes and the content organizations run. These are different systems of record, kept distinct on purpose.</p><div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground"><MaturityBadge maturity={area.provenance.maturity} /><span>Data source: {area.provenance.source}</span><span>Freshness: {area.provenance.freshness}</span></div></div>
    <WarningList warnings={area.warnings} />

    <section className="border border-border bg-card" aria-labelledby="content-review-queue-title" aria-busy={Boolean(busyReview)}>
      <div className="border-b border-border px-4 py-4 sm:px-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">Agent loop · human terminal</p><h2 id="content-review-queue-title" className="mt-1 text-lg font-semibold tracking-tight text-foreground">Opportunity content review</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">Review the exact source-linked brief produced by the content worker. Approval changes only the generated projection; canonical opportunity facts remain untouched.</p></div><span className={`inline-flex items-center gap-2 border px-2 py-1 text-xs font-medium ${data.reviewQueue.available ? 'border-information/30 bg-information-subtle text-information' : 'border-warning/30 bg-warning-subtle text-warning'}`}><span className="size-1.5 rounded-full bg-current" aria-hidden="true" />{data.reviewQueue.available ? 'Durable queue' : 'Queue unavailable'}</span></div></div>
    ...
    <section className="border border-dashed border-border bg-card p-4" aria-labelledby="planned-cms-title"><h2 id="planned-cms-title" className="text-sm font-semibold text-foreground">CMS capabilities not persisted yet</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">The registry deliberately does not render fake organization-owned editorial controls. These capabilities need durable models, permissions, revisions, and audit contracts.</p><ul className="mt-3 grid gap-2 text-xs text-muted-foreground sm:grid-cols-2">{data.planned.map((item) => <li key={item} className="border border-border p-3">{item}</li>)}</ul></section>

    <AlertDialog open={Boolean(pendingReviewRow)} onOpenChange={(open) => { if (!open) setPendingReview(undefined); }}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{pendingReview?.decision === 'approved' ? `Approve “${pendingReviewRow?.title}”?` : `Block “${pendingReviewRow?.title}”?`}</AlertDialogTitle>
          <AlertDialogDescription>{pendingReview?.decision === 'approved' ? 'This records a human approval for the generated brief and requests a publisher-lane handoff. It does not change canonical opportunity facts or confirm publication.' : 'This records a human block decision for the generated brief and keeps it out of public reads. Canonical opportunity facts remain untouched.'}</AlertDialogDescription>
        </AlertDialogHeader>
        {pendingReviewRow && <dl className="grid gap-2 border border-border bg-muted/30 p-3 text-xs"><div><dt className="font-medium text-muted-foreground">Review job</dt><dd className="mt-0.5 break-words text-foreground">{pendingReviewRow.jobId}</dd></div><div><dt className="font-medium text-muted-foreground">Opportunity</dt><dd className="mt-0.5 break-words text-foreground">{pendingReviewRow.opportunityId}</dd></div><div><dt className="font-medium text-muted-foreground">Organization</dt><dd className="mt-0.5 text-foreground">{pendingReviewRow.organizationName ?? 'Not confirmed'}</dd></div><div><dt className="font-medium text-muted-foreground">Review evidence</dt><dd className="mt-0.5 text-foreground">{pendingReviewRow.reviewReasons.join(' · ') || 'No review reasons recorded'}</dd></div><div><dt className="font-medium text-muted-foreground">Known unknowns</dt><dd className="mt-0.5 text-foreground">{pendingReviewRow.content.unknowns.join(' · ') || 'None recorded'}</dd></div><div><dt className="font-medium text-muted-foreground">Decision note</dt><dd className="mt-0.5 text-foreground">{reviewNotes[pendingReviewRow.jobId]?.trim() || 'No note provided'}</dd></div></dl>}
        <AlertDialogFooter>
          <AlertDialogCancel>Cancel</AlertDialogCancel>
          <AlertDialogAction variant={pendingReview?.decision === 'blocked' ? 'destructive' : 'default'} onClick={() => { if (!pendingReview) return; const review = pendingReview; setPendingReview(undefined); void commitReview(review.jobId, review.decision); }}>{pendingReview?.decision === 'approved' ? 'Record approval' : 'Record block'}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  </div>;
