...
export default function PlatformAdminCrm({ area }: { area: AdminArea<PlatformAdminCrmData> }) {
  ...
  async function addContact(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      const response = await fetch('/api/admin/crm', { method: 'POST', headers: { 'content-type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ kind: 'contact', organizationId, name: contactName, email: contactEmail, role: contactRole }) });
      const result = await response.json().catch(() => ({}));
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to save contact'); } finally { setSaving(false); }
  }
  ...
  async function addTask(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      const response = await fetch('/api/admin/crm', { method: 'POST', headers: { 'content-type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ kind: 'task', organizationId, title: taskTitle, description: taskDescription, dueAt: taskDueAt ? new Date(`${taskDueAt}T12:00:00Z`).toISOString() : undefined }) });
      const result = await response.json().catch(() => ({}));
      ...
      setTaskTitle(''); setTaskDescription(''); setTaskDueAt('');
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to save task'); } finally { setSaving(false); }
  }

  async function updateTask(taskId: string, status: 'open' | 'done') {
    setMessage(undefined);
    try {
      const response = await fetch('/api/admin/crm', { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ taskId, status }) });
      const result = await response.json().catch(() => ({}));
    ...
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to update task'); }
  }
  ...
  return <div className="space-y-8">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label="CRM summary">
      ...
      <MetricCard label="Contacts" value={area.data.summary.contacts} detail="Durable organization contacts" />
      <MetricCard label="Open follow-ups" value={area.data.summary.openTasks} detail={`${area.data.summary.tasks} total CRM tasks`} />
    </section>
    ...
    <section className="grid gap-5 lg:grid-cols-2" aria-label="CRM contacts and follow-ups">
      <div className="border border-border bg-white"><div className="border-b border-border px-4 py-4 sm:px-5"><h2 className="text-lg font-semibold tracking-tight text-foreground">Contacts</h2><p className="mt-1 text-xs text-muted-foreground">Explicitly assigned organization contacts; no global account is silently mapped.</p></div>{area.data.contacts.length === 0 ? <p className="px-5 py-10 text-center text-sm text-muted-foreground">No CRM contacts recorded.</p> : <div className="divide-y divide-border">{area.data.contacts.map((contact) => <article key={contact.id} className="px-4 py-4 sm:px-5"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium text-foreground">{contact.name}</p><p className="mt-1 text-xs text-muted-foreground">{contact.role ?? 'No role'} · {contact.status}</p></div>{contact.email && <a className="text-xs text-accent-deep underline underline-offset-4" href={`mailto:${contact.email}`}>{contact.email}</a>}</div><p className="mt-2 font-mono text-[11px] text-muted-foreground">{contact.organizationId ?? contact.accountId}</p></article>)}</div>}</div>
      <div className="border border-border bg-white"><div className="border-b border-border px-4 py-4 sm:px-5"><h2 className="text-lg font-semibold tracking-tight text-foreground">Follow-ups</h2><p className="mt-1 text-xs text-muted-foreground">Operator-owned tasks with explicit due dates and completion state.</p></div>{area.data.tasks.length === 0 ? <p className="px-5 py-10 text-center text-sm text-muted-foreground">No CRM follow-ups recorded.</p> : <div className="divide-y divide-border">{area.data.tasks.map((task) => <article key={task.id} className="flex items-start justify-between gap-3 px-4 py-4 sm:px-5"><div className="min-w-0"><p className={`text-sm font-medium ${task.status === 'done' ? 'text-muted-foreground line-through' : 'text-foreground'}`}>{task.title}</p><p className="mt-1 text-xs text-muted-foreground">{task.status} · {task.dueAt ? dateLabel(task.dueAt) : 'No due date'}</p>{task.description && <p className="mt-2 text-xs leading-5 text-muted-foreground">{task.description}</p>}</div>{task.status === 'done' ? <button type="button" onClick={() => updateTask(task.id, 'open')} className="shrink-0 text-xs font-medium text-accent-deep underline underline-offset-4">Reopen</button> : <button type="button" onClick={() => updateTask(task.id, 'done')} className="shrink-0 border border-border px-2.5 py-1.5 text-xs font-medium hover:border-primary hover:text-primary">Mark done</button>}</article>)}</div>}</div>
    </section>
    <section className="grid gap-5 lg:grid-cols-2" aria-label="CRM create records">
      <form onSubmit={addContact} className="border border-border bg-white p-5"><h2 className="text-lg font-semibold tracking-tight text-foreground">Add contact</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="block"><span className="text-xs font-medium">Name</span><input required value={contactName} onChange={(event) => setContactName(event.target.value)} className="mt-1 h-10 w-full border border-border px-3 text-sm" placeholder="Jane Doe" /></label><label className="block"><span className="text-xs font-medium">Email</span><input type="email" value={contactEmail} onChange={(event) => setContactEmail(event.target.value)} className="mt-1 h-10 w-full border border-border px-3 text-sm" placeholder="jane@example.org" /></label></div><label className="mt-3 block"><span className="text-xs font-medium">Role</span><input value={contactRole} onChange={(event) => setContactRole(event.target.value)} className="mt-1 h-10 w-full border border-border px-3 text-sm" placeholder="Programme director" /></label><button type="submit" disabled={saving || !area.data.available} className="mt-4 min-h-10 bg-foreground px-4 text-sm font-medium text-white disabled:opacity-50">Record contact</button></form>
      <form onSubmit={addTask} className="border border-border bg-white p-5"><h2 className="text-lg font-semibold tracking-tight text-foreground">Add follow-up</h2><label className="mt-4 block"><span className="text-xs font-medium">Title</span><input required value={taskTitle} onChange={(event) => setTaskTitle(event.target.value)} className="mt-1 h-10 w-full border border-border px-3 text-sm" placeholder="Send onboarding pack" /></label><label className="mt-3 block"><span className="text-xs font-medium">Due date</span><input type="date" value={taskDueAt} onChange={(event) => setTaskDueAt(event.target.value)} className="mt-1 h-10 w-full border border-border px-3 text-sm" /></label><label className="mt-3 block"><span className="text-xs font-medium">Description</span><textarea value={taskDescription} onChange={(event) => setTaskDescription(event.target.value)} rows={3} className="mt-1 w-full border border-border px-3 py-2 text-sm" placeholder="What should happen next?" /></label><button type="submit" disabled={saving || !area.data.available} className="mt-4 min-h-10 bg-foreground px-4 text-sm font-medium text-white disabled:opacity-50">Create follow-up</button></form>
    </section>
