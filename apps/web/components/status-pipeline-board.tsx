        ...
        return (
          <div key={stage} className="mt-6">
            ...
            <div className="mt-2 space-y-2">
              {items.map((item) => (
                <div key={item.opportunityId} className="flex items-start justify-between gap-4 rounded-lg border border-border bg-card p-3">
                  <div>
                    <h3 className="font-medium text-foreground">{item.title}</h3>
                    <p className="text-sm text-muted-foreground">
                      {item.organizationName ?? ''} · opportunity: {item.opportunityStatus}
                      {item.deadline ? ` · deadline ${item.deadline} (${item.daysToDeadline}d)` : ''}
