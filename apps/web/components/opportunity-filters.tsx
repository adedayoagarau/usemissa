...
export function OpportunityFilters({ locations, activeFilterCount, saveSearch }: FilterProps) {
  ...
  const practiceId = selectedForFacet(searchParams, 'practice-family');
  const genreId = selectedForFacet(searchParams, 'discipline');
  const styleId = selectedForFacet(searchParams, 'genre');

  const practiceOptions = useMemo(() => termsForBrowseLayer('discipline'), []);
  const disciplineOptions = useMemo(
    () => practiceId ? termsForBrowseLayer('genre', practiceId) : [],
    [practiceId],
  );
  const styleOptions = useMemo(
    () => genreId ? termsForBrowseLayer('style', genreId) : [],
    [genreId],
  );
  ...
  return (
    <div className={styles.filterRow}>
      <div className="flex flex-wrap items-center gap-2">
        {control('Discipline', <Palette className="size-4 text-muted-foreground" />, practiceId, practiceOptions.map((term) => ({ id: term.id, label: term.preferredLabel })), 'practice-family')}
        {control('Genre', <Tag className="size-4 text-muted-foreground" />, genreId, disciplineOptions.map((term) => ({ id: term.id, label: term.preferredLabel })), 'discipline', !practiceId)}
        {control('Style', <Tag className="size-4 text-muted-foreground" />, styleId, styleOptions.map((term) => ({ id: term.id, label: term.preferredLabel })), 'genre', !genreId)}
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          <label className={styles.toggle}><input type="checkbox" checked={searchParams.get('verified') === '1'} onChange={(event) => update('verified', '1', event.target.checked)} /><span className={styles.toggleTrack} />Fresh source only</label>
      ...
      <div className="mt-2 flex min-h-5 items-center justify-between gap-3" aria-live="polite">
        <p className="text-[11px] text-muted-foreground">{practiceId ? `Showing ${optionLabel(practiceId)}${genreId ? ` · ${optionLabel(genreId)}` : ''}${styleId ? ` · ${optionLabel(styleId)}` : ''}` : 'Start with a discipline to narrow the field.'}</p>
        ...
        {!pending && (practiceId || genreId || styleId) && <p className="inline-flex items-center gap-1 text-[11px] text-green"><Check className="size-3.5" />Results refreshed</p>}
      </div>
    </div>
  );
}
