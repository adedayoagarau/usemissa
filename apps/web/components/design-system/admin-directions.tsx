...
function AdminHeader({ navOpen, onNavOpen }: { navOpen: boolean; onNavOpen: () => void }) {
  return <header className={styles.adminHeader}><div><button type='button' className={styles.navButton} aria-expanded={navOpen} aria-controls='admin-mobile-navigation' onClick={onNavOpen}><Menu aria-hidden='true' /><span className='sr-only'>Toggle Admin navigation</span></button><a href='#admin-main'><strong>Missa</strong><span>Platform Admin</span></a></div><div className={styles.headerSearch}><Search aria-hidden='true' /><span>Search Admin</span><kbd>⌘ K</kbd></div><div><Button type='button' variant='ghost' size='icon' aria-label='Open Admin alerts'><Inbox aria-hidden='true' /></Button><button type='button' className={styles.operator} aria-label='Open operator account'>PA</button></div></header>
}
