...
function attentionFor(fixture: Fixture, role: Role): AttentionItem[] {
  ...
  if (fixture === 'taxonomy-conflict') return [{ id: 'taxonomy', title: 'A draft cannot be published yet', detail: 'One creative-practice rule conflicts with a more specific choice. Eligibility and form questions are unaffected.', scope: 'New Voices Residency', action: 'Review practice rules', tone: 'danger', icon: AlertCircle }]
...
function AppHeader({ onOpenCommand }: { onOpenCommand: () => void }) {
  return <header className={styles.appHeader}><a href='#organization-content' className={styles.wordmark}>Missa</a><nav aria-label='Product navigation'><a href='#'>Profile</a><a href='#' aria-current='page'>Organization</a></nav><div className={styles.headerActions}><Button type='button' variant='outline' size='sm' onClick={onOpenCommand}><Search aria-hidden='true' />Search<span className={styles.shortcut}>⌘K</span></Button><Button type='button' variant='ghost' size='icon' aria-label='Open Inbox'><Bell aria-hidden='true' /></Button><button type='button' className={styles.avatar} aria-label='Open Profile'>AO</button></div></header>
}
