...
const fixtureOptions: Array<{ value: Fixture; label: string }> = [
  ...
  { value: 'incomplete-basics', label: 'Incomplete Basics' }, { value: 'unknown-type', label: 'Unknown imported type' }, { value: 'deadlines', label: 'Deadline modes' }, { value: 'fee-unknown', label: 'Fee unknown' }, { value: 'fee-free', label: 'Explicitly free' }, { value: 'multi-currency', label: 'Multiple currencies' }, { value: 'practice-rules', label: 'Practice rules' }, { value: 'taxonomy-conflict', label: 'Taxonomy conflict' }, { value: 'deprecated-term', label: 'Deprecated taxonomy term' }, { value: 'large-rules', label: 'Large rule selection' }, { value: 'eligibility-conflict', label: 'Eligibility conflict' }, { value: 'geography-conflict', label: 'Geography conflict' },
...
const builderSections: Array<{ id: BuilderSection; label: string; description: string; icon: typeof Sparkles }> = [
  ...
  { id: 'practice', label: 'Practice rules', description: 'What creative work is accepted', icon: ListFilter },
...
const baseRows: OpportunityRow[] = [
  ...
  { id: 'opp-residency', title: 'New Voices Residency', program: 'Residencies', team: 'Programs', type: 'Residency', state: 'Draft', attention: 'Practice rules need review', deadline: 'Not decided', submissions: null },
...
function AppHeader() {
  return <header className={styles.appHeader}><a href='#opportunity-content' className={styles.skipLink}>Skip to content</a><a href='#' className={styles.wordmark}>Missa</a><nav aria-label='Product navigation'><a href='#'>Profile</a><a href='#' aria-current='page'>Organization</a></nav><div className={styles.headerActions}><Button type='button' variant='outline' size='sm'><Search aria-hidden='true' />Search<span>⌘K</span></Button><button type='button' className={styles.avatar} aria-label='Open Profile'>AO</button></div></header>
}
...
function ListEmpty({ fixture, onStatus }: { fixture: Fixture; onStatus: (message: string) => void }) {
  ...
  return <section className={styles.emptyState}><Sparkles aria-hidden='true' /><h2>Create the first Opportunity</h2><p>Start with the public facts, then review practice rules, eligibility, place, dates, fees, terms, and the submission form separately.</p><Button type='button' onClick={() => onStatus('New Opportunity builder opened in Basics.')}>Create Opportunity</Button></section>
}
...
const practiceTerms = [
  { id: 'taxterm_practice-writing', label: 'Writing and literature', facet: 'Practice family', rule: 'Accepted' },
...
function PracticeSection({ fixture, onStatus }: { fixture: Fixture; onStatus: (message: string) => void }) {
  ...
  return <div className={styles.practiceSection}><Alert><Info aria-hidden='true' /><AlertTitle>Practice rules describe accepted creative work</AlertTitle><AlertDescription>Opportunity type, eligibility, place, fees, and application materials are edited in their own sections.</AlertDescription></Alert>{fixture === 'taxonomy-conflict' ? <Alert variant='destructive'><AlertCircle aria-hidden='true' /><AlertTitle>Two practice rules conflict</AlertTitle><AlertDescription>Writing and literature is excluded while Poetry beneath it is preferred. Resolve the intended consequence before publishing.</AlertDescription><Button type='button' variant='outline' onClick={() => onStatus('Practice-rule conflict opened for resolution.')}>Resolve conflict</Button></Alert> : null}{fixture === 'deprecated-term' ? <Alert><RefreshCw aria-hidden='true' /><AlertTitle>One saved term needs review</AlertTitle><AlertDescription>The stable saved value is preserved. Choose the intended current term; Missa will not silently replace it.</AlertDescription></Alert> : null}<label className={styles.taxonomySearch}><span>Find a practice term</span><div><Search aria-hidden='true' /><Input placeholder='Search all relevant facets and aliases' /></div></label><section className={styles.ruleList}><header><div><h3>Selected practice rules</h3><p>{terms.length} rules across independent facets</p></div>{fixture === 'large-rules' ? <Button type='button' variant='outline'>Find conflicts</Button> : null}</header>{terms.map((term) => <article key={term.id}><div><strong>{term.label}</strong><span>{term.facet}</span></div><select aria-label={`Rule for ${term.label}`} defaultValue={term.rule}><option>Accepted</option><option>Preferred</option><option>Required</option><option>Excluded</option>{term.rule === 'Review' ? <option>Review</option> : null}</select><Button type='button' variant='ghost' size='icon' aria-label={`Remove ${term.label}`}><Trash2 aria-hidden='true' /></Button></article>)}</section></div>
}
...
function readinessIssues(fixture: Fixture): string[] {
  ...
  if (fixture === 'taxonomy-conflict') return ['Resolve the conflicting Writing and Poetry practice rules.']
  ...
  if (fixture === 'publish-blocked') return ['Choose an Opportunity type.', 'Resolve one practice-rule conflict.', 'Confirm the application fee.', 'Add a required Work field.']
