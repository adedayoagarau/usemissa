...
function ReviewBar({ direction, setDirection, fixture, setFixture }: { direction: Direction; setDirection: (direction: Direction) => void; fixture: Fixture; setFixture: (fixture: Fixture) => void }) {
  return <div className={styles.reviewBar} aria-label='Design review controls'><div className={styles.directionButtons}>{directions.map((item) => <button key={item.id} type='button' data-active={direction === item.id} onClick={() => setDirection(item.id)}><span>{item.number}</span>{item.title}</button>)}</div><label><span>Edge state</span><select value={fixture} onChange={(event) => setFixture(event.target.value as Fixture)}><option value='active'>Current Calendar</option><option value='empty'>First-use empty</option><option value='undated'>All timing undated</option><option value='conflict'>Conflicting dates</option><option value='moved'>Deadline moved</option><option value='dense'>Dense month</option><option value='timezone'>Timezone boundary</option><option value='feed'>Feed disconnected</option></select></label></div>
}

function SelectedReviewBar({ fixture, setFixture }: { fixture: Fixture; setFixture: (fixture: Fixture) => void }) {
  return <div className={styles.selectedReviewBar} aria-label='Design review controls'><div><strong>Selected Calendar composition</strong><span>Month + Agenda · mobile Agenda fallback</span></div><label><span>Edge state</span><select value={fixture} onChange={(event) => setFixture(event.target.value as Fixture)}><option value='active'>Current Calendar</option><option value='empty'>First-use empty</option><option value='undated'>All timing undated</option><option value='conflict'>Conflicting dates</option><option value='moved'>Deadline moved</option><option value='dense'>Dense month</option><option value='timezone'>Timezone boundary</option><option value='feed'>Feed disconnected</option></select></label></div>
}
...
function TrackerHeading({ onFeed, headingLevel = 'h2' }: { onFeed: () => void; headingLevel?: 'h1' | 'h2' }) {
  ...
  return <><div className={styles.pageHeading}><div><p className={styles.eyebrow}>Tracker schedule</p><Heading>Calendar</Heading><p>Exact opportunity deadlines, submission events, estimated responses, and timing that still needs review.</p></div><Button type='button' variant='outline' onClick={onFeed}><CalendarSync aria-hidden='true' />Calendar subscription</Button></div><nav className={styles.trackerViews} aria-label='Tracker views'><button type='button'>Next actions</button><button type='button'>Submission records</button><button type='button' aria-current='page'>Deadlines</button><button type='button'>By Work</button></nav></>
}
