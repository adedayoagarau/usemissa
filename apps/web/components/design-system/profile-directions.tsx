...
const sectionDefinitions: SectionDefinition[] = [
  ...
  { id: 'preferences', label: 'Preferences', description: 'Private practice and opportunity choices', icon: SlidersHorizontal },
...
const fixtureOptions: Array<{ value: Fixture; label: string }> = [
  ...
  { value: 'multi', label: 'Multi-practice creator' },
  { value: 'preference-conflict', label: 'Preference conflict' },
  { value: 'deprecated', label: 'Deprecated practice term' },
...
const baseTerms = [
  { id: 'writing', label: 'Writing and literature', facet: 'Practice', preference: 'Show me opportunities like this' },
...
const multiTerms = [
  ...baseTerms,
  { id: 'film', label: 'Film and moving image', facet: 'Practice', preference: 'Show me opportunities like this' },
...
function AppHeader() {
  return <header className={styles.appHeader}><a href='#profile-content' className={styles.wordmark}>Missa</a><nav aria-label='Primary navigation'><a href='#'>Opportunities</a><a href='#'>Tracker</a><a href='#'>Library</a></nav><div className={styles.headerActions}><Button type='button' variant='ghost' size='icon' aria-label='Open Inbox'><Inbox aria-hidden='true' /></Button><Button type='button' variant='ghost' size='icon' className={styles.mobileMenu} aria-label='Open navigation'><Menu aria-hidden='true' /></Button><button type='button' className={styles.avatar} aria-label='Open Profile' aria-current='page'>AO</button></div></header>
}
...
function Overview({ fixture, onNavigate }: { fixture: Fixture; onNavigate: (section: ProfileSection) => void }) {
  const next = fixture === 'new' ? { title: 'Add private opportunity preferences', copy: 'Choose a broad practice and the opportunity types you want to see. You can refine them later.', section: 'preferences' as const } : fixture === 'partial' ? { title: 'Finish geography and opportunity types', copy: 'These private settings help explain why an opportunity appears without deciding eligibility for you.', section: 'preferences' as const } : fixture === 'integration-attention' ? { title: 'Repair your Gmail connection', copy: 'Missa stopped reading new submission updates. Your confirmed Tracker history is unchanged.', section: 'integrations' as const } : { title: 'Review your public Profile', copy: 'Your name, short bio, and one selected Work are visible. Private preferences and Tracker history are not.', section: 'privacy' as const }
