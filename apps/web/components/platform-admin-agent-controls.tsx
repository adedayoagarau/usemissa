...
export default function PlatformAdminAgentControls({ area }: { area: AdminArea<PlatformAdminAgentControlsData> }) {
  ...
  async function requestControl(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      captureProductEvent('admin_agent_control_requested', { targetType, action });
      setTargetId('');
      setReason('');
    } catch (error) {
  ...
  return <div className="space-y-8">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label="Worker lifecycle summary"><MetricCard label="Runs observed" value={area.data.summary.runs} detail="Recent durable worker runs" /><MetricCard label="Running" value={area.data.summary.running} detail="Heartbeat-controlled runs" /><MetricCard label="Paused" value={area.data.summary.paused} detail="Operator-paused runs" /><MetricCard label="Stale" value={area.data.summary.stale} detail="Running without a recent heartbeat" /></section>
    <section className="border border-border bg-white" aria-labelledby="agent-runs-title"><div className="border-b border-border px-4 py-4 sm:px-5"><h2 id="agent-runs-title" className="text-lg font-semibold tracking-tight text-foreground">Live worker runs</h2><p className="mt-1 text-xs text-muted-foreground">Lifecycle status is worker-owned. A stale run is a recovery signal, not proof that work is lost.</p></div>{area.data.runs.length === 0 ? <p className="px-5 py-10 text-center text-sm text-muted-foreground">No durable worker runs recorded.</p> : <div className="divide-y divide-border">{area.data.runs.map((run) => <article key={run.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-5"><div className="min-w-0"><p className="text-sm font-medium text-foreground">{run.agentKind} · <span className={run.stale ? 'text-red-700' : run.status === 'paused' ? 'text-amber-700' : 'text-foreground'}>{run.status}{run.stale ? ' · stale' : ''}</span></p><p className="mt-1 truncate font-mono text-[11px] text-muted-foreground">{run.id} · {run.inputCount} in / {run.outputCount} out</p>{run.error && <p className="mt-1 truncate text-xs text-red-700">{run.error}</p>}</div><button type="button" onClick={() => { setTargetType('agent-run'); setTargetId(run.id); }} className="min-h-9 border border-border px-3 text-xs font-medium text-foreground hover:border-primary hover:text-primary">Control run</button></article>)}</div>}</section>
