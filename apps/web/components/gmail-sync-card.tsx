...
function GmailConnectLink() {
  // This endpoint starts an OAuth redirect, so it must use a full browser navigation.
  // eslint-disable-next-line @next/next/no-html-link-for-pages
  return <a className="inline-flex min-h-11 items-center justify-center rounded-lg bg-primary px-3 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/80 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50" href="/api/me/email-sync/gmail/start">Connect Gmail <ExternalLink className="ml-2 size-4" aria-hidden="true" /></a>;
}
export function GmailSyncCard() {
  ...
  return <Card id="gmail-sync" className="bg-white"><CardHeader><div className="flex flex-wrap items-start justify-between gap-4"><div><CardTitle>Gmail Sync</CardTitle><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Missa scans likely submission emails. You review updates before they reach your Tracker.</p></div><ShieldCheck className="size-5 text-green" aria-label="Review-first privacy control" /></div></CardHeader><CardContent className="space-y-5">
    {!gmail.connected ? <div className="space-y-4"><div className="rounded-lg border border-border bg-white p-4"><p className="font-medium text-foreground">Review before import</p><p className="mt-1 text-sm leading-6 text-muted-foreground">Missa reads only the window and labels you choose, stores sanitized excerpts for 30 days, and never shares your mail history with organizations. Attachments and raw messages are not stored.</p></div><GmailConnectLink /><p className="text-xs leading-5 text-muted-foreground">Google may describe Gmail read access as sensitive or restricted. You can disconnect at any time.</p></div> : <>
      ...
      {gmail.lastErrorCode && <p role="alert" className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">Gmail needs attention. Reconnect to resume syncing.</p>}
      ...
      {gmail.mode === 'autopilot' && <div className="rounded-lg border border-green/30 bg-green/5 p-4 text-sm leading-6 text-foreground"><p className="font-medium">Autopilot is on</p><p className="mt-1 text-muted-foreground"><Check className="mr-1 inline size-4 text-green" aria-hidden="true" />Only an exact Opportunity match and a clear received or in-review message can update Tracker. Everything else stays in Inbox for review.</p></div>}
      <Dialog open={autopilotOpen} onOpenChange={(open) => { setAutopilotOpen(open); if (!open) setAutopilotConfirmed(false); }}>
        <DialogContent>
          ...
          <div className="space-y-3 text-sm leading-6 text-foreground">
            <p>It can update a tracked Opportunity only when the match is exact and the message clearly says <span className="font-medium">received</span> or <span className="font-medium">in review</span>.</p>
