...
type Detail = {
  run: Pick<
    Run,
    "id" | "sourceId" | "trigger" | "mode" | "status" | "createdAt"
  >;
...
export default function IngestionV2Workbench() {
  ...
  const loadRuns = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/admin/ingestion-v2", {
        cache: "no-store",
      });
      const payload = await response.json();
      setSources(payload.sources ?? []);
      if (!response.ok)
        throw new Error(payload.error ?? "Unable to load v2 runs");
      setRuns(payload.runs ?? []);
      if (payload.warning) setMessage(payload.warning);
      captureProductEvent("ingestion_workbench_viewed", {
        run_count: payload.runs?.length ?? 0,
        selected_run: Boolean(searchParams.get("run")),
        data_maturity: "durable",
      });
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "Unable to load v2 runs",
      );
      captureProductEvent("ingestion_workbench_error", {
        surface: "run-list",
        error_code: "load-failed",
      });
    } finally {
      setLoading(false);
    }
  ...
  const selectRun = useCallback(
    async (run: Run) => {
      ...
      router.replace(`/admin/ingestion-v2?${nextParams.toString()}`, {
        scroll: false,
      });
      captureProductEvent("ingestion_run_selected", {
        source_id: run.sourceId,
        status: run.status,
        quality_decision: run.qualityDecision,
        failure_code: run.failureCode ?? "none",
      });
      const response = await fetch(
        `/api/admin/ingestion-v2?run=${encodeURIComponent(run.id)}`,
        { cache: "no-store" },
      );
      const payload = await response.json();
      if (!response.ok) {
  ...
  async function requestRun(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok)
        throw new Error(payload.error ?? "The shadow run could not be queued");
      setMessage(
        `Accepted ${payload.runId}. Worker status will appear after the next refresh.`,
      );
      captureProductEvent("ingestion_shadow_run_requested", {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "The shadow run could not be queued",
      );
      captureProductEvent("ingestion_workbench_error", {
        surface: "shadow-run-request",
  ...
  async function requestLaneBatch(
    lane: "core-daily" | "scheduled" | "single-run",
  ) {
    ...
    try {
      const response = await fetch("/api/admin/ingestion-v2", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ scope: "lane", lane, limit: 1000 }),
      });
      const payload = await response.json();
      if (!response.ok)
        throw new Error(
          payload.error ?? "The source batch could not be queued",
        );
      setMessage(
        `Accepted ${payload.batchId}: ${payload.queuedCount} ${lane} sources queued. The worker will continue on each source cadence.`,
      );
      captureProductEvent("ingestion_shadow_batch_requested", {
        scope: "lane",
        lane,
        queued_count: payload.queuedCount,
        request_result: "accepted",
      });
      await loadRuns();
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
          : "The source batch could not be queued",
      );
      captureProductEvent("ingestion_workbench_error", {
        surface: "shadow-lane-request",
        error_code: "request-failed",
      });
    } finally {
  ...
  useEffect(() => {
    ...
    const requestedRun = requestedId
    ...
    () =>
      runs.filter((run) => {
        const matchesStatus =
          status === "all" ||
          run.status === status ||
          (status === "review" && run.qualityDecision === "review");
        const matchesSource = sourceId === "all" || run.sourceId === sourceId;
        const haystack =
          `${run.id} ${run.sourceId} ${run.status} ${run.failureCode ?? ""}`.toLowerCase();
        return (
          matchesStatus &&
          matchesSource &&
  ...
  const counts = useMemo(
    () => ({
      ...
      review: runs.filter((run) => run.qualityDecision === "review").length,
  ...
  return (
    <div className="space-y-6">
      <section
        ...
        aria-label="Ingestion v2 state"
      >
        {[
          {
            label: "Failed runs",
            value: counts.failed,
            detail: "Needs diagnosis",
          },
          {
            label: "Review quality",
            value: counts.review,
            detail: "Not promotion proof",
          },
          {
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2
              id="ingestion-contract-title"
              className="mt-1 text-sm font-semibold"
            >
              Fetched → extracted → reviewed → published
            </h2>
            <p className="mt-1 max-w-3xl text-xs leading-5 text-muted-foreground">
              Nothing here publishes an opportunity or replaces source evidence.
              Stable sources run automatically; seasonal windows stay dormant
              until open.
            </p>
            <p className="mt-2 text-xs text-muted-foreground">
              {sources.filter((source) => source.eligible !== false).length}{" "}
              eligible ·{" "}
              {
                sources.filter(
                  (source) => source.schedule?.lane === "core-daily",
                ).length
              }{" "}
              daily core ·{" "}
              {
                sources.filter(
                  (source) => source.schedule?.lane === "scheduled",
                ).length
              }{" "}
              scheduled ·{" "}
              {
                sources.filter(
                  (source) => source.schedule?.lane === "single-run",
                ).length
              }{" "}
              single-run ·{" "}
              {sources.filter((source) => source.eligible === false).length}{" "}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              <select
                ...
                value={
                  sourceId === "all"
                    ? (sources.find((source) => source.eligible !== false)
                        ?.id ?? "")
                    : sourceId
                }
        ...
        <div className="min-w-0 border border-border bg-white">
          ...
          {loading ? (
            <p className="p-8 text-center text-sm text-muted-foreground">
              Loading durable run history…
            </p>
          ) : filteredRuns.length === 0 ? (
            <p className="p-8 text-center text-sm text-muted-foreground">
              No runs match this view. Empty is not the same as healthy.
            </p>
          ) : (
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full border border-border px-2 py-0.5 text-[11px] text-muted-foreground">
                        quality {run.qualityDecision}
                      </span>
                    </span>
                    <span className="mt-2 block truncate text-sm font-medium text-foreground">
                      {sources.find((source) => source.id === run.sourceId)
                        ?.name ?? run.sourceId}
                    </span>
                    <span className="mt-1 block text-xs text-muted-foreground">
                      {run.failureCode ?? "No failure"} · {run.snapshotCount}{" "}
                      snapshots · {run.fieldCount} fields ·{" "}
            ...
            <div className="flex min-h-[360px] items-center justify-center p-8 text-center">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-muted-foreground">
                  The inspector will show bounded source evidence, field
                  provenance, destinations, and warnings.
                </p>
              </div>
            </div>
          ) : !detail ? (
            ...
            <div>
                ...
                <dl className="mt-4 grid grid-cols-2 gap-3 text-xs">
                  <div>
                    ...
                    <dd className="mt-1 font-mono">
                      {detail.source.adapterId ?? "not observed"}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">Mode</dt>
                    <dd className="mt-1">{detail.run.mode} · unpublished</dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">Started</dt>
                    <dd className="mt-1">{formatTime(detail.run.createdAt)}</dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">Quality</dt>
                    <dd className="mt-1">
                      {detail.quality.decision} · {detail.quality.score}
                    </dd>
                  </div>
                </dl>
              </div>
              <div className="divide-y divide-border">
                <section className="p-4">
                  <h3 className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Publisher review</h3>
                  {detail.publisher ? (
                    <>
                      <div className="mt-2 flex flex-wrap gap-2">
                        <StatusBadge value={detail.publisher.decision} />
                        <span className="rounded-full border border-border px-2 py-0.5 text-[11px] text-muted-foreground">{detail.publisher.model}</span>
                  ...
                  ) : <p className="mt-2 text-xs text-muted-foreground">Publisher review has not run.</p>}
                </section>
                <section className="p-4">
                ...
                <section className="p-4">
                  ...
                  <div className="mt-3 space-y-2">
                    {detail.snapshots.map((snapshot) => (
                      <a
                        key={snapshot.id}
                        href={snapshot.finalUrl || snapshot.url}
                        target="_blank"
                        rel="noreferrer"
                        onClick={() =>
                          captureProductEvent("ingestion_run_evidence_viewed", {
                            section: "snapshots",
                            source_id: detail.run.sourceId,
                      ...
                      >
                        <span className="flex items-center justify-between gap-2 text-xs font-medium">
                          <span>
                            {snapshot.statusCode} ·{" "}
                            {snapshot.rendered ? "rendered" : "fetched"}
                          </span>
                          <ExternalLink
                            className="size-3.5"
                            aria-hidden="true"
                ...
                <section className="p-4">
                  <h3 className="text-xs font-semibold tracking-[0.12em] text-muted-foreground uppercase">
                    Extraction · {detail.fields.length} fields
                  </h3>
                  <div className="mt-3 divide-y divide-border border border-border">
                    {detail.fields.slice(0, 12).map((field) => (
                      <div
                        key={`${field.provenance.snapshotId}:${field.fieldName}`}
                        className="p-3"
                      >
                        ...
                        <p className="mt-1 text-xs break-words text-muted-foreground">
                          {field.rawValue ?? "unknown"}
                        </p>
                        <p className="mt-1 truncate font-mono text-[10px] text-muted-foreground">
                          {field.provenance.method} ·{" "}
                          {field.provenance.snapshotId}
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                <section className="p-4">
                  <h3 className="text-xs font-semibold tracking-[0.12em] text-muted-foreground uppercase">
                    Destinations · {detail.candidateLinks.length}
                  </h3>
                  <ul className="mt-3 space-y-2 text-xs">
                    {detail.candidateLinks.map((link) => (
                      <li key={`${link.role}:${link.url}`}>
                        <a
                          href={link.url}
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-center gap-2 text-accent-deep underline underline-offset-2"
                        >
                          <span className="rounded bg-muted px-1.5 py-0.5 font-mono text-[10px] no-underline">
                            {link.role}
                          </span>
                  ...
                  {detail.candidateLinks.length === 0 && (
                    <p className="text-xs text-muted-foreground">
                      No destination candidates observed.
                    </p>
                  )}
                </section>
                <section className="p-4">
