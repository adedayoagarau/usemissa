...
type List = { id: string; name: string; revision?: number };
...
export function ListPicker({ opportunityId, enabled, compact = false }: { opportunityId: string; enabled: boolean; compact?: boolean }) {
  ...
  const toggle = (list: List) => startTransition(async () => { const removing = has(list.id); const method = removing ? 'DELETE' : 'POST'; const res = await fetch(`/api/me/lists/${list.id}/opportunities/${opportunityId}`, { method, headers: list.revision ? { 'Idempotency-Key': crypto.randomUUID(), 'If-Match': String(list.revision) } : undefined }); const data = await res.json().catch(() => ({})); if (!res.ok) { toast.error(data.error ?? 'Could not update List'); return; } if (data.revision) setLists((current) => current.map((candidate) => candidate.id === list.id ? { ...candidate, revision: data.revision } : candidate)); setMemberships((current) => removing ? current.filter((membership) => !(membership.listId === list.id && membership.opportunityId === opportunityId)) : [...current, { listId: list.id, opportunityId }]); toast.success(removing ? `Removed from ${list.name}` : `Added to ${list.name}`); });
  const create = () => { const name = window.prompt('Name this List'); if (!name?.trim()) return; startTransition(async () => { const res = await fetch('/api/me/lists', { method: 'POST', headers: { 'content-type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ name }) }); const list = await res.json().catch(() => null); if (!res.ok || !list?.id) { toast.error(list?.error ?? 'Could not create List'); return; } setLists((current) => [list, ...current]); }); };
