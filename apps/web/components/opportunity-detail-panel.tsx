    ...
    <>
      ...
      <aside role={mobileOpen ? 'dialog' : 'complementary'} aria-modal={mobileOpen || undefined} aria-labelledby="opportunity-detail-title" className={`flex min-h-0 flex-col border-l border-border bg-card lg:sticky lg:top-0 lg:h-[calc(100vh-3.75rem)] lg:overflow-y-auto ${styles.detailPanel} ${mobileOpen ? styles.detailPanelMobileOpen : ''}`}>
        ...
        {activeTab === 'overview' && <div id="opportunity-panel-overview" role="tabpanel" className="space-y-6 p-6">
          ...
          <section><h3 className="text-base font-semibold text-foreground">Why this is a strong fit</h3>{reasons.length ? <ul className="mt-3 space-y-3">{reasons.map((reason, index) => <li key={`${reason.code}-${index}`} className="flex items-start gap-2 text-sm leading-6 text-muted-foreground"><Check className="mt-1 size-4 shrink-0 text-green" />{reason.label}</li>)}</ul> : <p className="mt-3 text-sm leading-6 text-muted-foreground">Complete your Passport profile to see why Missa recommends this opportunity for you.</p>}</section>
        </div>}
        ...
        <div className="mt-auto space-y-4 border-t border-border p-6"><p className="text-sm text-muted-foreground">Review your materials and submit on the next page.</p><div className="space-y-2">{opportunity.submissionAvailable && opportunity.submissionUrl ? <Button nativeButton={false} render={<Link href={`/api/opportunities/${opportunity.id}/submission`} />} className="h-11 w-full justify-between">Go to submission <ArrowRight className="size-4" /></Button> : <Button disabled className="w-full">Submission link unavailable</Button>}{opportunity.guidelinesUrl && <a href={opportunity.guidelinesUrl} target="_blank" rel="noreferrer" className="flex h-10 items-center justify-center gap-2 rounded-lg border border-border text-sm text-foreground hover:bg-muted">Official guidelines <ExternalLink className="size-3.5" /></a>}</div>{userId && <div className="flex flex-wrap items-center gap-3 border-t border-border pt-4">{opportunity.personal?.tracked ? <Button size="sm" variant="outline" disabled>Tracked</Button> : <TrackButton userId={userId} opportunityId={opportunity.id} />}<BookmarkButton userId={userId} opportunityId={opportunity.id} initialBookmarked={opportunity.personal?.bookmarked} />{opportunity.organizationId && !opportunity.personal?.followingOrganization && <FollowButton userId={userId} organizationId={opportunity.organizationId} organizationName={opportunity.organizationName} />}</div>}</div>
      </aside>
    </>
  );
}
