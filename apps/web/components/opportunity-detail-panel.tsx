  ...
  return (
    <aside className={`flex min-h-0 flex-col border-l border-border bg-card lg:sticky lg:top-0 lg:h-[calc(100vh-3.75rem)] lg:overflow-y-auto ${styles.detailPanel} ${mobileOpen ? styles.detailPanelMobileOpen : ''}`}>
      ...
      <div id="overview" className="space-y-6 p-6">
        ...
        <section><h3 className="text-base font-semibold text-foreground">Why this is a strong fit</h3>{reasons.length ? <ul className="mt-3 space-y-3">{reasons.map((reason) => <li key={reason.code} className="flex items-start gap-2 text-sm leading-6 text-muted-foreground"><Check className="mt-1 size-4 shrink-0 text-green" />{reason.label}</li>)}</ul> : <p className="mt-3 text-sm leading-6 text-muted-foreground">Complete your profile to see why Missa recommends this opportunity for you.</p>}</section>
