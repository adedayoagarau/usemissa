...
const queueOrder: PlatformAdminQueueName[] = [
  "source-health",
  "verification",
  "claims",
  "review",
...
function normalizeQueueFilter(value?: string): QueueFilter {
  if (value === "workspace" || value === "agents" || value === "outbox")
    return value;
  if (queueOrder.includes(value as PlatformAdminQueueName))
    return value as PlatformAdminQueueName;
  return "all";
}
...
function MaturityTag({ maturity }: { maturity: AdminMaturity }) {
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${maturityStyles[maturity]}`}
    >
...
function SeverityMark({ row }: { row: PlatformAdminQueueRow }) {
  ...
  return (
    <span
      ...
      title={`${humanize(row.severity)} severity`}
    >
      <Icon className={`size-4 ${color}`} aria-hidden="true" />
      <span className="sr-only">{humanize(row.severity)} severity</span>
    </span>
  );
}
...
function QueueAction({ action }: { action?: PlatformAdminQueueAction }) {
  if (!action)
    return <span className="text-xs text-muted-foreground">Inspect</span>;
  if (action.type === "link") {
    ...
    return (
      <Link
        ...
        className="inline-flex min-h-9 items-center gap-1.5 text-xs font-medium whitespace-nowrap text-accent-deep underline decoration-accent-tint underline-offset-4 hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
      >
        {action.label}
...
function DetailValue({ value, href }: { value: string; href?: string }) {
  if (!href)
    return (
      <span className="text-right font-mono text-[11px] break-words text-foreground">
        {value}
      </span>
    );
      ...
      >
        <div className="flex min-h-64 flex-col items-center justify-center text-center">
          ...
          <p className="mt-1 max-w-xs text-xs leading-5 text-muted-foreground">
            Choose a row to inspect its evidence, provenance, and safe next
            action.
          </p>
        </div>
      </aside>
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h2 id="queue-detail-title" tabIndex={-1} className="mt-3 text-lg font-semibold tracking-tight break-words text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary">
            {row.title}
          </h2>
          {row.subtitle && (
      ...
      <section className="mt-5 border-t border-border pt-4">
        ...
        <dl className="mt-3 space-y-2.5">
          {row.detail.evidence.map((item) => (
            <div
              key={item.label}
              className="grid grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)] gap-3 text-xs"
            >
              <dt className="text-muted-foreground">{item.label}</dt>
              <dd className="text-right font-mono text-[11px] text-foreground">
                {item.value.includes("T") ? formatUtc(item.value) : item.value}
              </dd>
            </div>
          ))}
        </dl>
      ...
      <section className="mt-5 border-t border-border pt-4">
        <h3 className="text-xs font-semibold text-foreground">Related IDs</h3>
        <dl className="mt-3 space-y-2.5">
          {row.detail.related.map((item) => (
            <div
            ...
            >
              <dt className="text-muted-foreground">{item.label}</dt>
              <dd>
                <DetailValue value={item.value} href={item.href} />
              </dd>
            </div>
          ))}
        </dl>
    ...
    <>
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full min-w-[820px] table-fixed text-left text-sm">
          <caption className="sr-only">Platform operations queue</caption>
          <thead className="border-b border-border bg-muted/30 text-[11px] text-muted-foreground">
            <tr>
              <th scope="col" className="w-[29%] px-4 py-3 font-medium">
                Item
              </th>
                ...
                >
                  ...
                  <td className="px-4 py-3 align-top">
                    ...
                    >
                      ...
                      {row.ageAt && (
                        <span className="mt-0.5 block font-mono text-[10px] whitespace-nowrap text-muted-foreground">
                          {formatUtc(row.ageAt)}
                        </span>
                      )}
                    </button>
...
function LaneHealth({
  ...
  const counts = queueOrder
    .map((queue) => ({
      queue,
      count: rows.filter((row) => row.queue === queue).length,
    }))
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2
            id="lane-health-title"
            className="text-sm font-semibold text-foreground"
          >
            Queue lenses
          </h2>
          <p className="mt-1 text-xs text-muted-foreground">
            Choose a lane to narrow the worklist. Counts reflect rows loaded for
            this read.
          </p>
        </div>
        <Link
          href="/admin/operations"
          className="text-xs font-medium text-accent-deep underline decoration-accent-tint underline-offset-4 hover:text-primary"
        >
          View all operations{" "}
...
export default function PlatformAdminOperationsQueue({
  area,
  initialQueue,
  initialSeverity,
  initialSearch,
  ...
  area: AdminArea<PlatformAdminOperationsData>;
  initialQueue?: string;
  ...
  const [queueFilter, setQueueFilter] = useState<QueueFilter>(
    normalizeQueueFilter(initialQueue),
  );
  const [severityFilter, setSeverityFilter] = useState<SeverityFilter>(
    normalizeSeverityFilter(initialSeverity),
  );
  const [selectedId, setSelectedId] = useState<string | undefined>(initialItem && data.queue.rows.some((row) => row.id === initialItem) ? initialItem : undefined);
  ...
  const workerStatus =
    data.worker.status === "unknown" ? "Unknown" : humanize(data.worker.status);
  const workerHealthy =
    data.worker.status === "running" || data.worker.status === "healthy";

  function clearFilters() {
    setSearch("");
    setQueueFilter("all");
    setSeverityFilter("all");
    replaceUrl({ q: undefined, queue: undefined, severity: undefined, item: undefined });
    setSelectedId(undefined);
  }

  function selectQueue(queue: QueueFilter) {
    setQueueFilter(queue);
    setSelectedId(undefined);
    replaceUrl({ queue: queue === "all" ? undefined : queue, item: undefined });
  }

  function replaceUrl(values: { q?: string; queue?: string; severity?: string; item?: string }) {
  ...
  function selectRow(id: string) {
    ...
    setSelectedId(id);
    replaceUrl({ item: id });
    if (window.matchMedia('(max-width: 1279px)').matches) window.setTimeout(() => document.getElementById('queue-detail-title')?.focus(), 0);
  }

  function closeDetail() {
    const previous = lastSelectedIdRef.current ?? selectedId;
    setSelectedId(undefined);
    replaceUrl({ item: undefined });
    if (previous) window.setTimeout(() => document.querySelector<HTMLElement>(`[data-queue-row="${CSS.escape(previous)}"]`)?.focus(), 0);
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div className="min-w-0">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
            ...
            <span>{maturityLabels[area.provenance.maturity]}</span>
            <span aria-hidden="true">·</span>
            <span>
              {area.provenance.freshness.replace("read at ", "Read at ")}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className={selectedRow ? "hidden xl:block" : undefined}><WarningList
        warnings={[
          ...area.warnings,
          ...data.durable.warnings.filter(
            (warning) => !area.warnings.includes(warning),
          ),
        ]}
      /></div>

      <section
        className={`${selectedRow ? "hidden xl:grid" : "grid"} border-y border-border sm:grid-cols-4`}
      ...
      >
        <div className="border-b border-border px-3 py-4 sm:border-r sm:border-b-0">
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">queue items</p>
        </div>
        <div className="border-b border-border px-3 py-4 sm:border-r sm:border-b-0">
        ...
        <div className="px-3 py-4">
          ...
          <p className="mt-1 font-mono text-2xl text-foreground tabular-nums">
            {data.queue.summary.oldest?.age ?? "—"}
          </p>
          <p className="mt-1 truncate text-[11px] text-muted-foreground">
            {data.queue.summary.oldest?.title ?? "No timestamped work"}
          </p>
        </div>
      </section>

      <div className={selectedRow ? "hidden xl:block" : undefined}><LaneHealth
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        >
          <div className="border-b border-border px-4 py-4 sm:px-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Select a row to inspect evidence and the safest next action.
                </p>
              </div>
          ...
          {filteredRows.length > 0 ? (
            ...
            <div className="flex min-h-64 flex-col items-center justify-center px-6 py-12 text-center">
              ...
              <p className="mt-1 max-w-sm text-xs leading-5 text-muted-foreground">
                {data.queue.summary.open === 0
                  ? "The current read model has no open verification, source, worker, durable, or organization work to act on."
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3 text-[11px] text-muted-foreground sm:px-5">
            <span>
              Showing {filteredRows.length} of {data.queue.summary.open} queue
              items
              {data.queue.summary.open > data.queue.rows.length
                ? " · each lane capped at 100"
                : ""}
            </span>
            <span className="font-mono">Read model · no private payloads</span>
          </div>
        </section>
        <div className={selectedRow ? "block" : "hidden xl:block"}><QueueDetail row={selectedRow} onClose={closeDetail} /></div>
      </div>
    </div>
  );
}
