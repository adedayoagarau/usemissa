...
export function PublisherClaimForm({
  ...
  const [profileId, setProfileId] = useState(
    preselectedProfileId || initialMagazines[0]?.profileId || "",
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        console.error(err);
        toast.error(
          "Could not submit claim request. Please check your connection.",
        );
      }
    });
  };
    ...
    return (
      <div className="rounded-2xl border border-primary/30 bg-primary/5 p-8 text-center sm:p-12">
        ...
        <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">
          ...
          <strong>{selectedMag?.name || "your publication"}</strong>. Our
          curatorial editorial team will review your masthead email and domain
          within 2 business days.
        </p>
        <div className="mt-6 flex justify-center gap-3">
          <Link
            href="/rankings/magazines"
            className="rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition-colors hover:bg-accent-deep"
          >
            Back to Rankings
          </Link>
          <Link
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <select
          ...
          className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
        >
        ...
        <p className="text-xs text-muted-foreground">
          Can&apos;t find your journal? Make sure your publication is listed in
          Missa&apos;s general directory.
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">
          Please use an institutional, university (.edu), or custom publication
          domain email whenever possible for fastest verification.
        </p>
      </div>
