...
export function SmartShortlistBuilder({
  ...
  const handleCopyMarkdown = () => {
    if (!plan) return;
    const titleHeader = pieceTitle.trim()
      ? `Submission Packet: "${pieceTitle.trim()}"`
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Tell Missa what you are submitting. We calculate a balanced batch
              of reach, target, and anchor journals to maximize placement and
              minimize wait times.
            </p>
          </div>
          ...
          <Button onClick={handleGenerate} disabled={isPending}>
            ...
            <span>
              {isPending ? "Calculating..." : "Re-Calculate Shortlist"}
            </span>
          </Button>
        </div>
            ...
            <div className="divide-y divide-border overflow-hidden rounded-xl border border-border bg-card shadow-sm">
              {plan.slots.map((slot) => {
                ...
                return (
                  <div
                    key={mag.profileId}
                    className="flex flex-col justify-between gap-4 p-5 transition-colors hover:bg-muted/30 sm:flex-row sm:items-center"
                  >
                    ...
                    <div className="flex min-w-0 items-start gap-3">
                      <span
                        className={cn(
                          ...
                          isReach &&
                            "border border-warning/20 bg-warning/10 text-warning dark:text-warning",
                          isTarget &&
                            "border border-information/20 bg-information/10 text-information dark:text-information",
                          !isReach &&
                            !isTarget &&
