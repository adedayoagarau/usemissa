...
export function OrganizationBilling({ organizationId, canManage }: { organizationId: string; canManage: boolean }) {
  ...
  async function start(plan: string) {
    ...
    if (!response.ok) {
      setError(body.error ?? 'We could not open billing. Try again.');
      return;
    }
  ...
  async function connect() {
    setError(null);
    const response = await fetch(`/api/orgs/${organizationId}/billing/connect`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok) {
      setError(body.error ?? 'We could not open payout setup. Try again.');
      return;
    }
    if (body.url) window.location.assign(body.url);
  }
  async function cancel() {
    ...
    const response = await fetch(`/api/orgs/${organizationId}/billing/cancel`, {
      method: 'POST',
    });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) {
      setError(body.error ?? 'We could not schedule this cancellation. Try again.');
      return;
    }
    ...
    <section className="rounded-lg border border-border bg-white p-5 shadow-sm" aria-labelledby="organization-billing-heading">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Upgrade when your submission volume needs more reviewers and programs.</p>
        </div>
      ...
      {canManage && (
        <div className="mt-4 flex flex-wrap gap-2">
          {billing.connectStatus !== 'connected' && (
            <button type="button" onClick={() => void connect()} className="rounded-md border border-border px-3 py-2 text-sm text-foreground">
              Connect payouts
            </button>
          )}
          {billing.plan === 'free' && (
            <>
              <button type="button" onClick={() => void start('pro')} className="rounded-md bg-foreground px-3 py-2 text-sm text-white">
                Upgrade to Pro
              </button>
              <button type="button" onClick={() => void start('program')} className="rounded-md border border-border px-3 py-2 text-sm text-foreground">
                Program plan
              </button>
            </>
          )}
