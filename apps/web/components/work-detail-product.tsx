  ...
  return (
    <article className={styles.work} aria-labelledby="work-title">
      ...
      {section === 'overview' ? <div className={styles.body}>
        ...
        <section>
          <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Work details</p><h2>How this Work is described</h2></div><Button type="button" variant="outline" onClick={openEditor}>Edit details</Button></div>
          {work.terms.length ? <dl className={styles.taxonomy}>{Object.entries(groupedTerms).map(([facet, terms]) => <div key={facet}><dt>{facet.replace(/-/gu, ' ')}</dt><dd>{terms.map((term) => term.label).join(' · ')}</dd></div>)}</dl> : <p className={styles.emptyCopy}>No details added yet. Add the terms that describe this Work; they do not decide eligibility or quality.</p>}
        </section>
      </div> : null}
      ...
      {section === 'practice' ? <div className={styles.body}><section><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Work details</p><h2>Describe the Work, not its eligibility</h2></div><Button type="button" onClick={openEditor}>Edit details</Button></div><p className={styles.explainer}>Missa stores stable selections and displays ordinary-language labels. No selected term becomes a quality score or eligibility verdict.</p>{work.terms.length ? <dl className={styles.taxonomy}>{Object.entries(groupedTerms).map(([facet, terms]) => <div key={facet}><dt>{facet.replace(/-/gu, ' ')}</dt><dd>{terms.map((term) => term.label).join(' · ')}</dd></div>)}</dl> : <p className={styles.emptyCopy}>No details added yet.</p>}</section></div> : null}
        ...
        <DialogContent className={styles.editDialog}>
          <form onSubmit={saveWork}>
            ...
            <div className={styles.formBody}>
              ...
              <fieldset><legend>Work details <span>Optional · up to 32</span></legend><TaxonomyBrowsePicker idPrefix={`edit-work-${work.id}`} selectedTermIds={termIds} onSelectedTermIdsChange={setTermIds} description="Choose the terms that describe the Work. These private selections do not decide eligibility or quality." /></fieldset>
