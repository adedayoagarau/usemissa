  ...
  return (
    <article className={styles.work} aria-labelledby="work-title">
      ...
      <nav className={styles.nav} aria-label="Work detail views">
        {(['overview', 'files', 'practice', 'history'] as WorkDetailSection[]).map((item) => <button key={item} type="button" aria-current={section === item ? 'page' : undefined} onClick={() => chooseSection(item)}>{item.slice(0, 1).toLocaleUpperCase() + item.slice(1)}</button>)}
      </nav>
      ...
      {section === 'overview' ? <div className={styles.body}>
        ...
        <section>
          <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Practice</p><h2>Canonical terms by facet</h2></div><Button type="button" variant="outline" onClick={openEditor}>Edit terms</Button></div>
          {work.terms.length ? <dl className={styles.taxonomy}>{Object.entries(groupedTerms).map(([facet, terms]) => <div key={facet}><dt>{facet.replace(/-/gu, ' ')}</dt><dd>{terms.map((term) => term.label).join(' · ')}</dd></div>)}</dl> : <p className={styles.emptyCopy}>No practice terms yet. Add only the facets that describe this Work; they do not decide eligibility or quality.</p>}
        </section>
      </div> : null}
        ...
        <DialogContent className={styles.editDialog}>
          <form onSubmit={saveWork}>
            ...
            <div className={styles.formBody}>
              ...
              <fieldset><legend>Practice terms <span>Optional · up to 32</span></legend><TaxonomyBrowsePicker idPrefix={`edit-work-${work.id}`} selectedTermIds={termIds} onSelectedTermIdsChange={setTermIds} description="Use independent facets to describe the Work. These private terms do not decide eligibility or quality." /></fieldset>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Delete “{work.title}”?</DialogTitle><DialogDescription>This permanently removes the private Work identity and its practice description. Separate Library files remain. Existing submission receipts are not rewritten.</DialogDescription></DialogHeader>
