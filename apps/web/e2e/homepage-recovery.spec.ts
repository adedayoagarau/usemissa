...
for (const width of [1440, 390])
  test(`recovered homepage ${width}`, async ({ browser }) => {
    ...
    await expect(page.locator(`img[src*="${plate}"]`).first()).toBeAttached();
    await page.locator("#homepage-hero-heading").scrollIntoViewIfNeeded();
    await page.screenshot({
      path: `/private/tmp/missa-recovered-home-${width}.png`,
      fullPage: true,
    });
    console.log(JSON.stringify({ width, errors }));
    expect(
      errors.filter((e) => /hydrat|didn't match|server rendered/i.test(e)),
    ).toEqual([]);
    expect(
      await page.evaluate(
...
test("current account screens are reused without writes", async ({ page }) => {
  const writes: string[] = [];
  page.on("request", (r) => {
    if (r.method() === "POST" && /api\/(auth|me\/onboarding)/.test(r.url()))
      writes.push(r.url());
  });
  ...
  await page.screenshot({
    path: "/private/tmp/missa-recovered-onboarding.png",
    fullPage: true,
  });
});
