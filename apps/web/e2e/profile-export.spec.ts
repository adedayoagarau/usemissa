...
async function trackFirstOpportunity(page: Page, userId: string) {
  ...
  const item = (
    (await opportunities.json()) as { items: Array<{ id: string }> }
  ).items[0];
  expect(item?.id).toBeTruthy();
  const tracked = await page.request.post(`/api/users/${userId}/track`, {
    data: { opportunityId: item.id },
  });
  expect(tracked.status()).toBe(201);
}

test("profile data export downloads tracker JSON and enforces an account cooldown", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Download all data" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Download Tracker CSV" }),
  ).toBeVisible();
  await expect(
    page.getByText(/Profile, Tracker, Works, Files, and Saved Answers/),
  ).toBeVisible();

  const json = await page.request.get(
    "/api/me/export?format=json&scope=tracker",
  );
...
test("export route includes private library scope", async ({ page }) => {
  ...
  const createdWork = await page.request.post("/api/me/library/works", {
    data: { title: "Night River" },
  });
  expect(createdWork.status()).toBe(201);
  const library = await page.request.get(
    "/api/me/export?format=json&scope=library",
  );
...
test("export is session-owned and rejects unauthenticated or user-selected scope", async ({
  page,
}) => {
  ...
  expect(ownBody.profile).toMatchObject({
    id: second.id,
    displayName: "Second Export User",
  });
  const selected = await page.request.get(
    `/api/me/export?userId=${encodeURIComponent(first.id)}`,
  );
