...
- `apps/web/app/api/cron/tick/route.ts` — the actual Vercel Cron target: validates `CRON_SECRET` (via `Authorization: Bearer` header or `?secret=` query param), calls the bounded worker tick, persists the result back to Postgres, and returns the tick report (sourcesChecked/changes/alerts counts). `RADAR_WORKER_BATCH_SIZE` controls the batch (default `10`, capped at `50`) so operators can increase throughput without an unbounded serverless request. The existing manual "Check for updates" button in `radar-engine/src/server/ui.ts` is untouched and still works for local/admin use.
...
- Full tick-success path was exercised against the live Neon database on 2026-08-04: a bounded 25-source tick completed, checked 25 sources, and persisted 156 checked/processed source rows. Nineteen sources returned fetch failures and were left stale rather than being marked processed. A binary PNG response also exposed a persistence boundary bug during the first rehearsal; `HttpFetcher` now rejects non-text content types and control-character payloads before snapshot persistence.
