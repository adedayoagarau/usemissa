...
Validation completed: Radar engine 68/68 tests, adapter tests 20 passing with the live Postgres test skipped, web typecheck, web lint (two pre-existing warnings only), production web build, a signed RSA Pub/Sub OIDC fixture, and a Playwright Gmail Sync consent flow. The live Google OAuth/Pub/Sub E2E remains environment-gated. The Pub/Sub endpoint now verifies Google OIDC issuer, audience, expiry, service-account identity, and signature when configured; local development retains the shared-secret fallback. Autopilot now uses an accessible shadcn dialog with an explicit acknowledgement checkbox and reversible copy.
